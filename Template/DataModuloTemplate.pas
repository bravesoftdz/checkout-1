...
uses
  ...
  ShellApi, Menus, BDE, TypInfo, uDebugEx, IBDatabase, IdComponent,
  ...
  IBCustomDataSet, IBQuery, IBSQL, ZConnection, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, ACBrBase, ACBrBAL, ZAbstractConnection;
...
type
  ...
  TDMTemplate = class(TDataModule)
    ...
    zServidor_Consulta: TZQuery;
    sqlConsulta: TQuery;
    sqlUpdate: TQuery;
    SQLProdutoSaldoAuxPSLDN3QTDE: TFloatField;
    SQLProdutoSaldoAuxPSLDN3QTDMIN: TFloatField;
    SQLProdutoSaldoAuxPSLDN3QTDMAX: TFloatField;
    ...
    SQLProdutoSaldoDiaPSDIN3QTDE: TFloatField;
...
uses WaitWindow, DataModulo, UnSoundPlay, UnitLibrary, TelaSplash;
      ...
      if Application.MessageBox(PChar(Email + Crlf + Crlf + ' Deseja reportar o erro?'), 'Erro', MB_YESNO + MB_ICONERROR + MB_SETFOREGROUND + MB_SYSTEMMODAL) = idYes  then
         begin
            ...
            if VerConexaoInterNet then
               begin
                 { ObjetoEmail := TIdSMTP.Create(Application);
                  ObjetoEmail.OnPacketSent := DM.MandandoEmail;
                  ObjetoEmail.OnSuccess    := DM.TerminaEmail;
                  ObjetoEmail.Host := 'smtp.conceitossistemas.com';
                  ObjetoEmail.UserID := 'conceito';
                  ObjetoEmail.PostMessage.ToAddress.Add('suporte@multistorers.com.br');
                  ObjetoEmail.PostMessage.Body.Add('Ocorreu um erro na Aplicação ' + Application.Title + CrLf + 'Detalhes: ' + CrLf + Email);
                  ObjetoEmail.PostMessage.FromName := 'Suporte Interno';
                  ObjetoEmail.PostMessage.Subject  := 'Relatorio de erros';
                  ObjetoEmail.PostMessage.Date     := FormatDateTime('dd/mm/yyyy hh:mm', Now);
                  ObjetoEmail.PostMessage.FromAddress := 'suporte@multistorers.com.br.com';
                  if FileExists(ImagemArquivo) then
                     ObjetoEmail.PostMessage.Attachments.Add(ImagemArquivo);
                  ObjetoEmail.Connect;
                  if ObjetoEmail.Connected then
                     ObjetoEmail.SendMail;
                  ObjetoEmail.Disconnect;
                  ObjetoEmail.Destroy; }
...
begin
   ...
   SQLAcesso.Close;
   SQLAcesso.SQL.Text := 'select distinct USPEA60NOMETELA from USUARIOPERMISSOES where USPEA60NOMETELA = ''' + Janela + '''';
   SQLAcesso.Open;
...
//  CorSelecaoGrid    := $00E0C8B0;
  ...
  if not DB.Connected then
    ...
    begin
      ...
      stlAlias := TStringList.Create;
      try
        DB.Session.GetAliasParams('Easy_Gestao',stlAlias);

        {try
          SQLCON.Close;
          SQLCON.ConnectionName := 'Gestao';
          SQLCON.Params.Values['DriverName'] := 'InterBase';
          SQLCON.Params.Values['DataBase'] := stlAlias.Values['SERVER NAME'];
          SQLCON.Open;
