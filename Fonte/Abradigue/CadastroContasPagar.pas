...
type
  TFormCadastroContasPagar = class(TFormCadastroTEMPLATE)
    ...
    SQLTemplateCTPGN3JUROPAGTO: TBCDField;
    SQLTemplateCTPGN3MULTAPAGTO: TBCDField;
    SQLTemplateCTPGN3DESCPAGTO: TBCDField;
    SQLTemplatePORTICOD: TIntegerField;
    SQLTemplatePDCPA13ID: TStringField;
    SQLTemplatePENDENTE: TStringField;
    SQLTemplateREGISTRO: TDateTimeField;
    SQLTemplateCTPGN2TOTPAG: TBCDField;
    GroupBox1: TGroupBox;
    DBGrid1: TDBGrid;
    ...
    SQLTemplateCTPGN2DESCPROMO: TBCDField;
    Label11: TLabel;
    EvDBNumEdit2: TEvDBNumEdit;
    SQLTemplateCTPGCPREVREAL: TStringField;
    SQLTemplateCTPGN3TXJURO: TBCDField;
    SQLTemplateCTPGN3TXMULTA: TBCDField;
    ...
    SQLEspelhoNovoLancCTPGN3VLR: TBCDField;
    ...
    SQLEspelhoNovoLancCTPGN2DESCPROMO: TBCDField;
    ...
    SQLTemplateCTPGN3VLR: TBCDField;
    Panel4: TPanel;
    Label13: TLabel;
    Label14: TLabel;
    ...
    RetornaPlanoContasCredito: TSpeedButton;
    DBEdit7: TDBEdit;
    DBEdit8: TDBEdit;
    ...
    procedure DBEditHistoricoPadraoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure RetornaPlanoContasCreditoClick(Sender: TObject);
...
uses DataModulo, CadastroPortador, CadastroTipoDocumento, CadastroFornecedor,
     UnitLibrary, CadastroPlanodeContas, TelaConsultaEmpresa,
  TelaConsultaPlanoContas, CadastroHistoricoPadrao, TelaConsultaPagamento,
  TelaAssistenteLancamentoContasPagar, WaitWindow;
...
begin
  ...
  SQLPortador.Open;
  PageControlPlanoContas.ActivePage := TabDebito;
end;
...
procedure TFormCadastroContasPagar.DBEdit7KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_F2 then
    RetornaPlanoContasCredito.Click ;
end;
...
procedure TFormCadastroContasPagar.RetornaPlanoContasCreditoClick(
  Sender: TObject);
begin
  inherited;
  FieldLookup := DsTemplate.DataSet.FieldByName('PLCTA15CODCREDITO');
  FieldOrigem := 'PLCTA15COD';
  TipoPlanoContas := 'C';
  CriaFormulario(TFormTelaConsultaPlanoContas, 'FormTelaConsultaPlanoContas', False, True, 'Contas à Pagar');
end;
...
begin
   try
     ...
     if SQLCount.RecordCount > 0 then
       begin
         if DM.SQLConfigFinanceiroOPBCICODESTORNODEB.IsNull then
           InformaErro('É necessário Configurar uma Operação Banária de Estorno "Débito" para o Contas A PAGAR no Configurador', True, Nil);

         if not Pergunta('Sim', 'Deseja Criar um Lançamento de Débito no "MOVIMENTO BANCÁRIO" Valor de R$ ' +
                     FormatFloat('#,##0.00', SQLCount.FieldByName('MVBCN2VLRDEB').AsFloat) + ' referennte o Estorno do Pagamento de ID : "' + SQLPagamentoCTPGA13ID.AsString + '"?' + #13 +
         ...
         LancaMovimentacaoBanco(SQLTemplateEMPRICOD.AsInteger, SQLCount.FieldByName('CTCRICOD').AsInteger,
                              ...
                              SQLCount.FieldByName('MVBCN2VLRDEB').AsFloat, SQLCount.FieldByName('MVBCDCHQBOMPARA').AsDateTime,
     ...
     if SQLCount.RecordCount > 0 then
       begin
         if DM.SQLConfigFinanceiroOPTEICODESTORNODEB.IsNull then
         ...
         if not Pergunta('Sim', 'Deseja Criar um Lançamento de Crédito na "TESOURARIA/CAIXA" no valor de R$ ' +
                         FormatFloat('#,##0.00', SQLCount.FieldByName('TESON2VLRDEBITO').AsFloat) + ' referennte o Estorno do Pagamento de ID : "' + SQLPagamentoCTPGA13ID.AsString + '"?' + #13 +
                         'Clique "SIM" para Continuar e "NÃO" para Cancelar o Estorno.') then
            Abort;
         ...
         LancaMovimentacaoTesouraria(SQLTemplateEMPRICOD.AsInteger, SQLCount.FieldByName('TERMICOD').AsInteger,
                                     ...
                                     SQLCount.FieldByName('TESON2VLRDEBITO').AsFloat,
                                     'Estorno de Crédito Referente Pagamento: ' + SQLTemplateCTPGINROPARC.AsString + '-' + SQLPagamentoPAGAICOD.AsString,
                                     SQLTemplateCTPGA13ID.AsString,'', '', SQLCount.FieldByName('CQEMA13ID').AsString,
...
{       SQLCount.Close;
       ...
       if not SQLTemplateCQEMA13ID.IsNull then
        begin
          ...
          DM.SQLTemplate.Close;
          DM.SQLTemplate.SQL.Clear;
          DM.SQLTemplate.SQL.Add('Update ContasPagar Set CQEMA13ID = "' + IDChequeEmitido + '" where CTPGA13ID = "' +  SQLTemplateCTPGA13ID.AsString + '" and CTPGCPREVREAL = "R"');
          DM.SQLTemplate.ExecSQL;
