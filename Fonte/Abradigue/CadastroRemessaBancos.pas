...
type
  TFormCadastroRemessaBancos = class(TFormCadastroTEMPLATE)
    ...
    SQLContaReceberCTRCN2VLR: TBCDField;
    SQLContaReceberCTRCN2DESCFIN: TBCDField;
    SQLContaReceberNUMEICOD: TIntegerField;
    SQLContaReceberPORTICOD: TIntegerField;
    SQLContaReceberCTRCN2TXJURO: TBCDField;
    SQLContaReceberCTRCN2TXMULTA: TBCDField;
    SQLContaReceberCTRCA5TIPOPADRAO: TStringField;
    SQLContaReceberCTRCDULTREC: TDateTimeField;
    SQLContaReceberCTRCN2TOTREC: TBCDField;
    SQLContaReceberCTRCN2TOTJUROREC: TBCDField;
    SQLContaReceberCTRCN2TOTMULTAREC: TBCDField;
    SQLContaReceberCTRCN2TOTDESCREC: TBCDField;
    SQLContaReceberCTRCN2TOTMULTACOBR: TBCDField;
    ...
    SQLTemplateRMBCN2VLRDIAATRASO: TBCDField;
    SQLTemplateRMBCDLIMDESCONTO: TDateTimeField;
    SQLTemplateRMBCN2VLRDESCONTO: TBCDField;
    ...
    MnADMRemBancodoBrasil: TMenuItem;
    ...
    procedure MnADMRemBancodoBrasilClick(Sender: TObject);
...
begin
  ...
  if (Sender as TRxSpeedButton).Name = 'Button3' then
    begin
      ...
      CriaFormulario(TFormCadastroRemessaBancosContasReceber,'FormCadastroRemessaBancosContasReceber', True, False, '');
    end;
end;
          ...
          Else
            Begin
              ...
              Else
                Status:='';
              End;
              DM.SQLTemplate.SQL.Text:='UPDATE REMESSABANCOCONTASRECEBER SET RBCRDSTATUS="'+Data+'",RBCRA60STATUS="'+Status+'",PENDENTE="S",REGISTRO="'+FormatDateTime('mm/dd/yyyy hh:mm:ss',dm.DataBaseSistema)+'" WHERE CTRCA13ID="'+CTRCA13ID+'" AND (RBCRDSTATUS IS NULL OR RBCRDSTATUS<="'+Data+'")';
...
begin
  ...
  CriaFormulario(TFormCadastroConfigRemessaBanco,'FormCadastroConfigRemessaBanco',False,True,'');
end;
      ...
      with gbTitulo do
        begin
          ...
          if DsTemplate.DataSet.FieldByName('RMBCA2SEGINSTRUCAO').AsString <> '' then
            SegundaInstrucaoCodificada := DsTemplate.DataSet.FieldByName('RMBCA2SEGINSTRUCAO').AsString;
            
          if SQLTemplate.FieldByName('RMBCA2PRIINSTRUCAO').AsString <> '' then
            PrimeiraInstrucaoCodificada := DsTemplate.DataSet.FieldByName('RMBCA2PRIINSTRUCAO').AsString;
          ...
          begin
             ...
             CodigoConvenio      := SQLConfigRemessa.FieldByName('CFRBA15CODCONVENIO').AsString;
             {Endereço do cedente}
             with Endereco do
             begin
...
procedure TFormCadastroRemessaBancos.MnADMRemBancodoBrasilClick(
  Sender: TObject);
begin
  inherited;
...
begin
  ...
  try
    ...
    MakeWindow(SQLTitulos.RecordCount,'Aguarde a geração do arquivo de remessa!');
    PrepareQuery(SQLConfigRemessa,'SELECT * FROM CONFIGREMESSABANCO WHERE CFRBA13ID = "' + DsTemplate.DataSet.FieldByName('CFRBA13ID').AsString + '"');
    AssignFile(Arq,SQLConfigRemessaCFRBTPATH.AsString + '\BANRI' + FormatDateTime('ddmmyy hhnnss',Now) + '.TXT');
    ReWrite(Arq);
    SQLEmpresa.Close;
