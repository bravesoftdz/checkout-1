...
type
  TFormTelaBaixaDocumentosReceberRecebimento = class(TForm)
    ...
    TblContaCorrente1CTCRN2SALDOATUAL: TBCDField;
    GroupBox9: TGroupBox;
    Label3: TLabel;
    Label7: TLabel;
...
begin
  ComboTipoLiquidacao.SetFocus;
  if DM.SQLConfigFinanceiroALINICODRECEBER.AsInteger > 0 then
    ComboAlinea.Value := DM.SQLConfigFinanceiroALINICODRECEBER.AsString;
  if DM.SQLConfigFinanceiroCGFIA254HISTRECEBE.AsString <> '' then
    begin
      MemoHistorico.Text      := DM.SQLConfigFinanceiroCGFIA254HISTRECEBE.AsString;
      MemoHistTesouraria.Text := DM.SQLConfigFinanceiroCGFIA254HISTRECEBE.AsString;
    end;
  if DM.SQLConfigFinanceiroNUMEICODRECEBER.AsInteger > 0 then
    begin
      ComboNumerario.Value      := DM.SQLConfigFinanceiroNUMEICODRECEBER.AsString;
      ComboNumerarioCheque.Value:= DM.SQLConfigFinanceiroNUMEICODRECEBER.AsString;
    end;
  if DM.SQLConfigFinanceiroOPBCICODRECEBER.AsInteger > 0 then
    ComboOperacaoBanco.Value := DM.SQLConfigFinanceiroOPBCICODRECEBER.AsString;
  if DM.SQLConfigFinanceiroOPTEICODRECEBER.AsInteger > 0 then
    begin
      ComboOperacaoTesouraria.Value      := DM.SQLConfigFinanceiroOPTEICODRECEBER.AsString;
    end;
  if DM.SQLConfigFinanceiroPORTICODRECEBER.AsInteger > 0 then
    ComboPortador.Value := DM.SQLConfigFinanceiroPORTICODRECEBER.AsString;
...
begin
  ...
  if DM.SQLConfigFinanceiroALINICODRECEBER.AsInteger > 0 then
    MemTblChequeRecebidoSituacao.Value := DM.SQLConfigFinanceiroALINICODRECEBER.AsInteger;
  if DM.SQLConfigFinanceiroNUMEICODRECEBER.AsInteger > 0 then
    begin
      MemTblChequeRecebidoNumerario.Value:= DM.SQLConfigFinanceiroNUMEICODRECEBER.AsInteger;
    end;
  if DM.SQLConfigFinanceiroPORTICODRECEBER.AsInteger > 0 then
    MemTblChequeRecebidoPortador.Value := DM.SQLConfigFinanceiroPORTICODRECEBER.AsInteger;
