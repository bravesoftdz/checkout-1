  ...
  while not SQLContasReceber.Eof do
    begin
      ...
      if VarValorPago > 0 then
        begin
          if VarValorPago >= SQLContasReceber.FieldByName('Saldo').AsFloat then
            begin
              TblRecebimento.FieldByName('Baixar').Value := True;
              VarValorPago := VarValorPago-SQLContasReceber.FieldByName('Saldo').AsFloat;
            end
          else
            if VarValorPago < SQLContasReceber.FieldByName('Saldo').AsFloat then
              begin
                TblRecebimento.FieldByName('Baixar').Value := True;
                TblRecebimentoValor.Value := VarValorPago;
                VarValorPago := 0;
              end;
        end;
              ...
              if TblRecebimento.FieldByName('Baixar').Value then
                begin
                 ...
                 if (Application.FindComponent('FormTelaBaixaDocumentosReceberRecebimento') <> nil) then
                  ...
                  if  (DM.SQLConfigFinanceiroCGFIUSATESOURARIA.AsString = 'S')
...
begin
  ...
  if ((TblRecebimento.Active) and
    ...
    begin
      ...
      FormCadastroRecibo.SQLTemplate.MacroByName('MFiltro').AsString := 'RECIDEMISSAO IS NULL AND RECICTIPO = "C"';
...
begin
  ...
  Case FormTelaPesquisarDocumentoReceber.ModalResult of
    Mrok : Begin
             if TblRecebimento.IsEmpty then
