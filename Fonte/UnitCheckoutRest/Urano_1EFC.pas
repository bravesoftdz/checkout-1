...
//  function  Envia_FP_Urano_1EFC(FP:string) : boolean ;
  ...
  function  FecharCupomFiscal_Urano_1EFC(TipoAcrescDesc : String; Valor, VlrAcrescDesc, VlrTroco : Double ; Ecf_CNFV, Msg : String; SQLFPVista, SQLFPPrazo : TRxQuery;NomeCli,EnderecoCli,CidadeCli,DocumentoCli, IDCupom:string) : boolean ;
...
begin
  ...
  begin
    if (SQLFPVista.Fieldbyname('TIPOPADR').AsString = 'CRD') or (SQLFPVista.Fieldbyname('TIPOPADR').AsString = 'CRTF') or
       (SQLFPVista.Fieldbyname('TIPOPADR').AsString = 'CRT') then
      begin
        if I = 1 then
          EfetuaFPCupom_Urano_1EFC(RetornaTotalizadorNumerarioECF(Ecf_ID, SQLFPVista.Fieldbyname('NUMEICOD').AsString),
                                   SQLFPVista.Fieldbyname('VALORPARC').AsFloat + VlrTroco, True)
        else
          EfetuaFPCupom_Urano_1EFC(RetornaTotalizadorNumerarioECF(Ecf_ID, SQLFPVista.Fieldbyname('NUMEICOD').AsString),
      ...
      begin
        if i = 1 then
          EfetuaFPCupom_Urano_1EFC(RetornaTotalizadorNumerarioECF(Ecf_ID, SQLFPVista.Fieldbyname('NUMEICOD').AsString),
                                   SQLFPVista.Fieldbyname('VALORPARC').AsFloat + VlrTroco, False)
        else
          EfetuaFPCupom_Urano_1EFC(RetornaTotalizadorNumerarioECF(Ecf_ID, SQLFPVista.Fieldbyname('NUMEICOD').AsString),
  ...
  if (Application.FindComponent('FormTelaFechamentoVenda') <> nil) and
     (DM.SQLTerminalAtivoTERMCIMPRDADINTCUP.Value <> 'N') then
       IdComprador(ConverteAcentos(NomeCli),
                   '0',
                   ConverteAcentos(DocumentoCli) + ' VD: ' + SQLLocate('CUPOM','CUPOA13ID','VENDICOD','"' + IDCupom + '"') +
                   ' PV: ' + SQLLocate('CUPOM','CUPOA13ID','PLRCICOD','"' + IDCupom + '"'),
