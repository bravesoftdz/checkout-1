...
uses
  ...
  RxMemDS, DateUtils, AdvOfficeStatusBar, AdvOfficeStatusBarStylers;
...
uses TelaItens, TelaTeclasAtalhoTelaRecebimetoCrediario,
     ...
     IMPNAOFISCAL, WindowsLibrary, WaitWindow, TelaDadosCheque, DataModuloImpNotaFiscal, ExtensoLib,
  VarSYS ;
    ...
    begin
      if (EstadoRecCred = InformandoDocumento) and (EntradaDados.text <> '') then
        begin
          ...
          if not IsNumeric(EntradaDados.text, 'INTEGER') then
            begin
          ...
          if (PortaECFAtual <> '') and (ECFAtual = '') then
            begin
              ...
              while not DM.SQLECF.Eof do
                begin
                  try
                    if AbrirGaveta(DM.SQLECFECFA30MODELO.AsString,PortaECFAtual) then DM.SQLECF.Last;
                  except
                    Application.ProcessMessages;
                  end;
                  DM.SQLECF.Next;
                  ...
                  if FileExists('ReciboFiscal.txt') then
                    begin
                      if Pergunta('SIM','Imprimir Recibo na Impressora Fiscal') = true then
                        begin
                          Repeat
                            TxtReciboECF := '-----------    R  E  C  I  B  O   -------------' +#13+#10+ #13+#10+
                                            'Recebi de: '+ SQLLocate('CLIENTE','CLIEA13ID','CLIEA60RAZAOSOC','"' + SQLParcelasReceberTempCLIEA13ID.asstring + '"') +#13+#10+#13+#10+
                                            'O Valor de R$ : '+FormatFloat('##0.00',TotalPagar.Value) +#13+#10+#13+#10+
                            ...
                            begin
                              ...
                              for i:= 1 to length(totalrecibo) do
                                ...
                                'Saldo Rest..: '+StrTm(FormatFloat('###,##0.00',TotalCliente - TotalRecbto),12,' ',0)+#13+#10;
                            end;
                            AbrirReciboFiscal(ECFAtual, TxtReciboECF);
              ...
              if Length(TotalRecibo) > 0 then
                begin
                  if pergunta('SIM','Deseja Imprimir Recibo?') then
                    begin
                      xIniFile := TIniFile.Create(DM.PathAplicacao+'IBGestao.ini');
...
procedure TFormTelaRecebimentoCrediario.ImgDesligarClick(Sender: TObject);
begin
  Close;
end;
