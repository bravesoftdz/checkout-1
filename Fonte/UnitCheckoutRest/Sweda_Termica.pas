...
//Funcoes Adilson Complementares-------------------------------------------------------------------------------------------------
  ...
  function AbreRecebimentoNaoFiscal_Sweda_Termica( Indice_do_Totalizador: String; Acrescimo_ou_Desconto: String; Tipo_Acrescimo_ou_Desconto: String; Valor_Acrescimo_ou_Desconto: String; Valor_do_Recebimento: String; Texto_Livre: String ): Integer; StdCall; External 'Daruma32.dll';
...
begin
  try
    ...
    if NotaGaucha = 'S' then
      begin
        repeat
          CpfOK := False;
          Documento := '';
          Documento := InputBox('Nota Fiscal Gaucha!','Informar CPF no Cupom?','');
          if Documento <> '' then
            if length(Documento)=11 then
              begin
                if ValidaCPF(Documento) then
                  CpfOK := True
                else
    ...
    IntRet := ECF_AbreCupom(pchar(Documento));
    ...
    begin
      ...
      case DM.SQLTerminalAtivoTERMINRODECQUANT.Value of
        2 : begin
              ...
              IntRet   := ECF_VendeItem(Codigo, Descricao, Aliquota, Tipo_de_Quantidade, QuantEcf, 2, ValorUnitarioECF, Tipo_de_Desconto, ValorDescontoEcf );
            end;
        3 : begin
              QuantEcf := FormatFloat('##0.000',Quantidade);
              IntRet   := ECF_VendeItemTresDecimais(Codigo, Descricao, Aliquota, QuantEcf, ValorUnitarioECF, Tipo_de_Desconto, '0,00' );
              ...
              while (TotalItemECF>TotalItemCalc) do
                begin
                  ValorUnitarioMenosDesconto := ValorUnitarioMenosDesconto + 0.001;
                  TotalItemCalc := Quantidade*ValorUnitarioMenosDesconto;
                end;
              QuantEcf := FormatFloat('##0.000', Quantidade);
              ValorUnitarioEcf   := FormatFloat('##0.00',ValorUnitarioMenosDesconto);
