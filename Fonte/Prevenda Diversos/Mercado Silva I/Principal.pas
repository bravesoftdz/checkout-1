...
begin
  try
    ...
    Linha := 1;
    rdp.Abrir;
    rdp.ImpF(Linha,1,'MERCADO SILVA',[Expandido, Normal]); Inc(Linha);
    rdp.Imp(Linha, 1,'RUA CRICIUMA, 537 - GRAVATAI - RS'); Inc(Linha,2);
    rdp.Imp(Linha, 1,'Controle.: '  + TblPedidoPedICod.AsString + '  Data: '  + TblPedidoDtEmissao.AsString); Inc(Linha);
    rdp.Imp(Linha, 1,'Operador.: '  + TblPreVendaCabUsuarioVendaSTR.AsString); Inc(Linha);
    ...
    rdp.Imp(Linha,10,'Acre/Desc   R$');
    rdp.ImpVal(Linha,39,'##0.00',TblPreVendaCabAcrescimo.Value-TblPedidoTotalDesconto.Value,[]);  Inc(Linha);
    rdp.Imp(Linha,10,'Total Geral R$');
    rdp.ImpVal(Linha,39,'##0.00',(TblPedidoTotal.Value+TblPreVendaCabAcrescimo.Value),[]);  Inc(Linha);
    // Forma Pgto
    TblPedidoFinanceiro.First;
    While not TblPedidoFinanceiro.eof Do
      Begin
        rdp.Imp(Linha,25,TblPedidoFinanceiroNumerario.Value);
        rdp.ImpVal(Linha,39,'##0.00',TblPedidoFinanceiroValor.Value,[]);  Inc(Linha);
