...
begin
  ...
  try
    ...
    TblPedidoDtEmissao.AsString              := FormatDateTime('dd/mm/yyyy hh:mm:ss',now);
    ...
    While Not TblPreVendaItem.Eof Do
      Begin
        ...
        TblPedidoItemCor.Value         := dm.SQLLocate('MARCA','MARCICOD','MARCA60DESCR',dm.SQLLocate('PRODUTO','PRODICOD','MARCICOD',TblPreVendaItemCodigo.AsString));
        TblPedidoItem.Post;
...
begin
    Linha := 1;
    RDprint.NumerodeCopias := 2;
    RDprint.Abrir;
    RDprint.ImpF(Linha, 1,'DISTRITO ENERGY ',[Expandido, Italico]); Inc(Linha,2);
    RDprint.ImpF(Linha, 1,'AV FERNANDO FERRARI, 515 - VILA CITY - CACHOEIRINHA/RS',[Normal]); Inc(Linha);
    RDprint.ImpF(Linha, 1,'Fone/Fax.: 51 3471-5448',[Normal]);
    RDprint.ImpF(Linha,50,'ORCAMENTO',[Normal]); Inc(Linha,2);
    ...
    RDprint.Imp(Linha,1,'---------------------------------------------------------------------------------------------');    Inc(Linha);
    RDprint.Imp(Linha,1,'Codigo Referencia   Descricao                        Marca   Un  Quant    Vlr.Unit  Vlr.Total');    Inc(Linha);
    RDprint.Imp(Linha,1,'---------------------------------------------------------------------------------------------');    Inc(Linha);
    TblPedidoItem.First;
    While not TblPedidoItem.eof Do
      Begin
        RDprint.Imp(Linha, 1,FormatFloat('#####00000',TblPedidoItemCodigo.value));
        if TblPedidoItemTecido.value <> '' then
          RDprint.Imp(Linha, 8,TblPedidoItemTecido.value);
        if TblPedidoItemTroca.value = 'S' then
          RDprint.Imp(Linha,21,'(T)'+copy(TblPedidoItemDescricao.AsString,1,29))
        else
          RDprint.Imp(Linha,21,copy(TblPedidoItemDescricao.AsString,1,32));
        if TblPedidoItemCor.AsString <> '' then
          RDprint.Imp(Linha,54,TblPedidoItemCor.AsString);
        if TblPedidoItemUn.AsString <> '' then
          RDprint.Imp(Linha,62,TblPedidoItemUn.AsString);
        RDprint.ImpVal(Linha,65,'##0.00',TblPedidoItemQtd1.Value,[]);
        RDprint.ImpVal(Linha,77,'##0.00',TblPedidoItemVlrUnitario.Value,[]);
    ...
    RDprint.Imp(Linha,68,'Total Itens');
