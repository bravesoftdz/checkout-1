...
var EmpresaNome, CodCliente, ImpCaixa, ImpressoraCozinha, nVias, Saltar : String;
...
begin
  try
    ...
    if TblPreVendaCabTipoVenda.Value = 'Tele' then
      begin
        rdp.ImpF(Linha, 1,'Tele Nro.: ' + IntToStr(TblPreVendaCabTicketNumero.asInteger),[Expandido, Normal]); Inc(Linha,2);
      end
    else
      begin
        rdp.Imp(Linha, 1,'Controle Nro.: '  + TblPreVendaCabTicketNumero.Value); Inc(Linha,1);
      end;
    ...
    if TblPreVendaCabTipoVenda.Value = 'Tele' then
      begin
        ...
        rdp.ImpF(Linha, 1,'Controle Nro.: ' + IntToStr(TblPreVendaCabTicketNumero.AsInteger),[Expandido, Normal]); Inc(Linha,2);
        rdp.Imp(Linha, 1,'Data........: '  + FormatDateTime('dd/mm/yy hh:nn:ss',now)); Inc(Linha);
        if TblPreVendaCabMotoboy.Value <> '' then
          rdp.Imp(Linha, 1,'Motoboy...: ' + TblPreVendaCabMotoboy.AsString); Inc(Linha);
        rdp.Imp(Linha, 1,'Cliente ..: '   + TblPreVendaCabNomeClienteVenda.AsString); Inc(Linha);
