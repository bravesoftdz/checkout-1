...
uses
  ...
  Dialogs, TelaGeralTEMPLATE, RxLookup, StdCtrls, Mask, DBCtrls, Buttons, jpeg,
  ExtCtrls, ToolEdit, CurrEdit, Grids, DBGrids, DB, DBTables, RxQuery, ComCtrls,
  ConerBtn, Menus, UCrpe32, RXCtrls, StrUtils, Placemnt, ppDB, ppDBPipe, ppDBBDE,
  ppComm, ppRelatv, ppProd, ppClass, ppReport, FormResources, AdvOfficeStatusBar,
...
type
  TFormTelaTransferencia = class(TFormTelaGeralTEMPLATE)
    ...
    rgOnOff: TRadioGroup;
    cdsTranferenciaOnline: TClientDataSet;
    cdsTranferenciaOnlineTRFEA13ID: TStringField;
    cdsTranferenciaOnlineEMPRICOD: TIntegerField;
    cdsTranferenciaOnlineTERMICOD: TIntegerField;
    cdsTranferenciaOnlineTRFEICOD: TIntegerField;
    cdsTranferenciaOnlineTRFEIEMPRDEST: TIntegerField;
    cdsTranferenciaOnlineTRFEDEMISSAO: TDateTimeField;
    cdsTranferenciaOnlineTRFEDRECEBIMENTO: TDateTimeField;
    cdsTranferenciaOnlineTRFECRECEBIDO: TStringField;
    cdsTranferenciaOnlineTRFEA30USUENVIO: TStringField;
    cdsTranferenciaOnlineTRFEA30USURECEB: TStringField;
    cdsTranferenciaOnlineREGISTRO: TDateTimeField;
    cdsTranferenciaOnlinePENDENTE: TStringField;
    cdsTranferenciaOnlineEmpresaRemetente: TStringField;
    cdsTranferenciaOnlineEmpresaDestino: TStringField;
    cdsTranferenciaOnlineTRFECDIVERG: TStringField;
    cdsTranferenciaOnlineEMPRICODORIGEM: TIntegerField;
    cdsTranferenciaItemOnline: TClientDataSet;
    cdsTranferenciaItemOnlineTRFEA13ID: TStringField;
    cdsTranferenciaItemOnlineTRITICOD: TIntegerField;
    cdsTranferenciaItemOnlinePRODICOD: TIntegerField;
    cdsTranferenciaItemOnlineTRITN3QTDEENVIADA: TFloatField;
    cdsTranferenciaItemOnlineTRITN3QTDERECEBIDA: TFloatField;
    cdsTranferenciaItemOnlineTRITA254DIVERGENCIA: TStringField;
    cdsTranferenciaItemOnlineREGISTRO: TDateTimeField;
    cdsTranferenciaItemOnlinePENDENTE: TStringField;
    cdsTranferenciaItemOnlineClassFiscal: TStringField;
    cdsTranferenciaItemOnlineMarca: TStringField;
    cdsTranferenciaItemOnlineUnidade: TStringField;
    cdsTranferenciaItemOnlineProdutoLookup: TStringField;
    cdsTranferenciaItemOnlineReferencia: TStringField;
    cdsTranferenciaItemOnlineCodBarras: TStringField;
    cdsTranferenciaItemOnlineTRITN3VLRCUSTO: TFloatField;
    cdsTranferenciaItemOnlineCUPOA13ID: TStringField;
    Label8: TLabel;
    BtnPedidos: TSpeedButton;
    SQLTranferenciaPDCPA13ID: TIntegerField;
    EditPedidoCompra: TEdit;
    ...
    procedure edQtdeProdutoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure edCodigoProdutoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure DBGridGeradasDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
  ...
  private
    ...
    Origem, Destino, OperacaoAcessada: string;
...
uses
  VarSYS, UnitLibrary, TelaSelecaoProduto, DataModulo,
  TelaTransferenciaMercadoriaReceberProduto, WaitWindow,
  TelaConsultaProdutoGeral, TelaTransferenciaGrade, CadastroPedidoCompra,
  uDlgTransferencia; //, CadastroPedidoCompra;
...
begin
 ...
 { if MatrizFilial = 'F' then  }
  ...
  if not SQLEmpresaDestino.Active then
    SQLEmpresaDestino.Active := True;
  ...
  if not SQLTranferencia.Active then
    SQLTranferencia.Active := True;
...
procedure TFormTelaTransferencia.SQLEmpresaRemetenteCalcFields(DataSet: TDataSet);
begin
  inherited;
...
begin
  ...
  begin
    Resultado := Application.MessageBox('Este Produto ja esta na lista de Transferência.' + #13 + 'Deseja adicionar a Quantidade à Quantidade da lista?' + #13 + 'Clique "NÃO" para Substituir a Quantidade ou Cancelar.', 'Item Duplicado', MB_YESNOCANCEL + MB_SETFOREGROUND + MB_ICONQUESTION + MB_DEFBUTTON1);
...
procedure TFormTelaTransferencia.SQLEmpresaDestinoCalcFields(DataSet: TDataSet);
begin
  inherited;
...
begin
  ...
  begin
    Informa('Não foi configurado as Operações de Estoque para Transferência.' + #13 + 'Configure no Configurador Cadastros/Parâmetros/Gerais e tente novamente.');
    Abort;
  end;
  ...
  if (rgOnOff.ItemIndex = 1) and (SQLTranferencia.MacroByName('MEmpresa').AsString = 'EMPRICOD = ' + EmpresaPadrao) and SQLTranferencia.Active then
  ...
  begin
    ...
    if dm.SQLTerminalAtivoTERMCMOVTRANSF.Value = 'S' then
    begin
           // Saida da Mercadoria da Empresa Origem
      GravaMovimentoEstoqueSimples(DM.SQLProduto, DM.SQLProdutoFilho, DM.SQLProdutoSaldo, dblEmpresaOrigem.KeyValue, tblTempTranferenciaProdutoCod.AsInteger, DM.SQLConfigGeral.FieldByName('OPESICODTRANSFSAID').AsInteger, tblTempTranferenciaProdutoQtde.AsFloat, 'S', FormatDateTime('mm/dd/yyyy', Now), '0', 'TRANSFERENCIA', SQLTranferenciaTRFEA13ID.AsString, '');
    end;
  ...
  begin
    ...
    begin
      ...
      SQLNotaFiscalItemNFITN2VLRLUCRO.asFloat := (SQLNotaFiscalItemNFITN2VLRUNIT.asFloat - StrToFloat(dm.SQLLocate('PRODUTO', 'PRODICOD', 'PRODN3VLRCOMPRA', SQLTranferenciaItemPRODICOD.AsString))) * SQLNotaFiscalItemNFITN3QUANT.asFloat;
      SQLNotaFiscalItemNFITN2VLRDESC.asFloat := (SQLNotaFiscalItemNFITN2VLRUNIT.AsFloat * SQLNotaFiscalItemNFITN3QUANT.asFloat) * (SQLNotaFiscalItemNFITN2PERCDESC.asFloat / 100);
      ...
      SQLNotaFiscalItemNFITICST.AsString := SQLLocate('PRODUTO', 'PRODICOD', 'PRODIORIGEM', SQLTranferenciaItemPRODICOD.AsString) + SQLLocate('PRODUTO', 'PRODICOD', 'PRODISITTRIB', SQLTranferenciaItemPRODICOD.AsString);
      ...
      if (CalculaIPI = 'S') and (SQLNotaFiscalItemNFITN2PERCIPI.AsFloat > 0) then
        SQLNotaFiscalItemNFITN2VLRIPI.asFloat := (SQLNotaFiscalItemNFITN2VLRUNIT.AsFloat * SQLNotaFiscalItemNFITN3QUANT.asFloat) - SQLNotaFiscalItemNFITN2VLRDESC.asFloat * (SQLNotaFiscalItemNFITN2PERCIPI.asFloat / 100);
      ...
      begin
        ...
        if SQLNotaFiscalItemNFITN2PERCREDUCAO.AsString = '' then
          SQLNotaFiscalItemNFITN2PERCREDUCAO.Value := 0;
        ...
        if SQLNotaFiscalItemNFITN2PERCSUBS.AsString = '' then
...
begin
  ...
  begin
    Informa('Não foi encontrado o relatório de Transferência.' + #13 + 'Arquivo Relatorio Transferencia-Envio.rpt em ' + DM.SQLConfigGeralCFGEA255PATHREPORT.AsString);
    Abort;
  end
  else
  begin
...
procedure TFormTelaTransferencia.DStblTempTranferenciaStateChange(Sender: TObject);
begin
  inherited;
...
procedure TFormTelaTransferencia.SQLTranferenciaBeforePost(DataSet: TDataSet);
begin
  inherited;
  ...
  if rgOnOff.ItemIndex = 1 then
  begin
    GravarDadosOnline(DataSet);
  end;

end;

procedure TFormTelaTransferencia.SQLTranferenciaItemBeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure TFormTelaTransferencia.SQLTranferenciaItemCalcFields(DataSet: TDataSet);
begin
  inherited;
  if DataSet.FieldByName('PRODICOD').AsVariant <> null then
  begin
...
procedure TFormTelaTransferencia.SQLTranferenciaItemNewRecord(DataSet: TDataSet);
begin
  inherited;
  SQLTranferenciaItemTRFEA13ID.AsString := SQLTranferenciaTRFEA13ID.AsString;
end;

procedure TFormTelaTransferencia.edQtdeProdutoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = Enter then
  begin
...
procedure TFormTelaTransferencia.TabSheetGerarTransferenciaShow(Sender: TObject);
begin
  inherited;
  edCodigoProduto.SetFocus;
end;
...
procedure TFormTelaTransferencia.DBGridTranferenciasDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
...
procedure TFormTelaTransferencia.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
  if gdSelected in State then
  begin
...
procedure TFormTelaTransferencia.edCodigoProdutoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TFormTelaTransferencia.btReceberTransferenciaClick(Sender: TObject);
var
  DivergenciasRel: TStringList;
  I: Integer;
begin
  inherited;
  ...
  if rgOnOff.ItemIndex = 0 then
    FormTelaTransferenciaMercadoriaReceberProduto.dsSQLItens.DataSet := SQLTranferenciaItem
  else
    FormTelaTransferenciaMercadoriaReceberProduto.dsSQLItens.DataSet := cdsTranferenciaItemOnline;
  ...
  begin
    ...
    begin
      Informa('Não foi encontrado o relatório de Divergências.' + #13 + 'Arquivo Relatorio Transferencia-Divergencias.rpt em ' + DM.SQLConfigGeralCFGEA255PATHREPORT.AsString);
      Abort;
    end
    else
    begin
...
procedure TFormTelaTransferencia.tblTempTranferenciaCalcFields(DataSet: TDataSet);
begin
  inherited;
...
procedure TFormTelaTransferencia.SQLTranferenciaAfterOpen(DataSet: TDataSet);
begin
  inherited;
  if not SQLTranferenciaItem.Active then
    SQLTranferenciaItem.Active := True;
...
procedure TFormTelaTransferencia.SQLEmpresaDestinoAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
begin
  ...
  begin
    OperacaoAcessada := TButton(Sender).Name;
    AbrirDadosOnline;
  end;
...
begin
  ...
  begin
    ...
    mnRecebimento.Enabled := (cdsTranferenciaOnlineTRFECDIVERG.AsString = 'S');
  end;
end;

procedure TFormTelaTransferencia.edQtdeProdutoKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
...
begin
  ...
  case RadioRecebida.ItemIndex of
    0:
      begin
        SQLTranferencia.MacroByName('MFiltro').AsString := 'TRFECRECEBIDO = "N"';
        OperacaoAcessada := 'Button2';
      end;
    1:
      begin
        SQLTranferencia.MacroByName('MFiltro').AsString := 'TRFECRECEBIDO = "S"';
        OperacaoAcessada := 'Button3';
      end;
    2:
      begin
        SQLTranferencia.MacroByName('MFiltro').AsString := '0=0';
        OperacaoAcessada := 'Button1';
      end;
  end;
  SQLTranferencia.Open;
end;

procedure TFormTelaTransferencia.DBGridGeradasDrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
  if TDBGrid(Sender).DataSource.DataSet.FindField('TRFECRECEBIDO').AsString = 'N' then
    TDBGrid(Sender).Canvas.Font.Color := $00332FCA
...
procedure TFormTelaTransferencia.DSSQLTranferenciaDataChange(Sender: TObject; Field: TField);
begin
  inherited;

  btReceberTransferencia.Enabled := (DSSQLTranferencia.DataSet.FieldByName('TRFECRECEBIDO').AsString = 'N') and (DSSQLTranferencia.DataSet.FieldByName('EMPRICOD').AsInteger <> EmpresaCorrente) and (PageControl.ActivePage = TabTransfRecebida);

end;

procedure TFormTelaTransferencia.SQLTranferenciaBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFormTelaTransferencia.SQLNotaFiscalBeforePost(DataSet: TDataSet);
var
  Erro: Boolean;
  i: integer;
begin
...
procedure TFormTelaTransferencia.edtCustoKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  if Key = #13 then
  begin
...
begin
  ...
  begin
    GravaMovimentoEstoqueSimples(DM.SQLProduto, DM.SQLProdutoFilho, DM.SQLProdutoSaldo, DSSQLTranferencia.DataSet.fieldbyname('TRFEIEMPRDEST').AsInteger, DSSQLTranferenciaItem.DataSet.fieldbyname('PRODICOD').AsInteger, DM.SQLConfigGeral.FieldByName('OPESICODTRANSFENTR').AsInteger, DSSQLTranferenciaItem.DataSet.fieldbyname('TRITN3QTDEENVIADA').AsFloat, 'E', FormatDateTime('mm/dd/yyyy', Now), '0', 'TRANSFERENCIA', DSSQLTranferenciaItem.DataSet.fieldbyname('TRFEA13ID').AsString, '');
...
procedure TFormTelaTransferencia.ImportarBematechDC20001Click(Sender: TObject);
var
...
procedure TFormTelaTransferencia.rgOnOffClick(Sender: TObject);
begin
  inherited;

  if rgOnOff.ItemIndex = 0 then
  begin
    DSSQLTranferencia.DataSet := SQLTranferencia;
    DSSQLTranferenciaItem.DataSet := SQLTranferenciaItem;
  end
  else
  begin
    DSSQLTranferencia.DataSet := cdsTranferenciaOnline;
    DSSQLTranferenciaItem.DataSet := cdsTranferenciaItemOnline;
  end;

end;

procedure TFormTelaTransferencia.AbrirDadosOnline;
var
  xsql: string;
  i: integer;
begin
  ...
  else if OperacaoAcessada = 'Button2' then
    xsql := xsql + ' and TRFECRECEBIDO = ''N'' '
  else if OperacaoAcessada = 'Button3' then
  ...
  try
    ...
    try
      ...
      on e: Exception do
      begin
        MessageDlg('Problema ao acessar o servidor.' + #13 + e.message, mtWarning, [mbOK], 0);
      end;
    end;
...
procedure TFormTelaTransferencia.GravarDadosOnline(pDados: TDataSet);
var
  xsql: string;
  i: integer;
begin
...
procedure TFormTelaTransferencia.cdsTranferenciaOnlineBeforePost(DataSet: TDataSet);
begin
  inherited;
  GravarDadosOnline(DataSet);
end;

procedure TFormTelaTransferencia.GravarDadosItemOnline(pDados: TDataSet);
var
  xsql: string;
  i: integer;
begin
  ...
  xsql := ' select * from TRANSFERENCIAITEM where TRITICOD = ' + QuotedStr(pDados.FieldByName('TRITICOD').AsString) + ' AND TRFEA13ID = ' + QuotedStr(pDados.FieldByName('TRFEA13ID').AsString);
...
procedure TFormTelaTransferencia.cdsTranferenciaItemOnlineBeforePost(DataSet: TDataSet);
begin
  inherited;
  if rgOnOff.ItemIndex = 1 then
    GravarDadosItemOnline(DataSet);
...
procedure TFormTelaTransferencia.cdsTranferenciaItemOnlineCalcFields(DataSet: TDataSet);
begin
  inherited;
  if DataSet.FieldByName('PRODICOD').AsVariant <> null then
  begin
...
procedure TFormTelaTransferencia.BtnPedidosClick(Sender: TObject);
begin
  inherited;
  EditLookup := EditPedidoCompra;
  FieldOrigem := 'PDCPA13ID';
  CriaFormulario(TFormCadastroPedidoCompra, 'FormCadastroPedidoCompra', False, True, False, '');
end;

procedure TFormTelaTransferencia.incluiProdutoPedido;
var
  item : Integer;
begin
  if not (fDlgTransferencia.mTransfPedido.IsEmpty) then
  begin
    MakeWindow(fDlgTransferencia.mTransfPedido.RecordCount, 'Aguarde - Transferindo dados!');
    try
     {TODO -ORussimar -CPendente : Rotina incluir produtos}
      if not tblTempTranferencia.Active then
        tblTempTranferencia.Active := True;
      fDlgTransferencia.mTransfPedido.First;
      while not fDlgTransferencia.mTransfPedido.Eof do
      begin
        if ((fDlgTransferencia.mTransfPedidoCodigo_Produto.AsInteger > 0) and (EncontrouProduto(fDlgTransferencia.mTransfPedidoCodigo_Produto.AsString, SQLProduto))) then
        begin
          SetProgress(Item);
          tblTempTranferencia.Append;
          tblTempTranferenciaProdutoCod.AsInteger := fDlgTransferencia.mTransfPedidoCodigo_Produto.AsInteger;
          tblTempTranferenciaProdutoQtde.AsFloat := fDlgTransferencia.mTransfPedidoQuantidade_Informada.AsFloat;
          tblTempTranferenciaProdutoCod.AsInteger := SQLProdutoPRODICOD.AsInteger;
          tblTempTranferenciaProdutoBarras.AsString := SQLProdutoPRODA60CODBAR.AsString;
          tblTempTranferenciaProdutoNome.AsString := SQLProdutoProdutoLookup.AsString;
          tblTempTranferenciaProdutoEstoque.AsFloat := StrToFloat(SQLLocate('PRODUTOSALDO', 'PRODICOD', 'PSLDN3QTDE', fDlgTransferencia.mTransfPedidoCodigo_Produto.AsString));
          tblTempTranferenciaProdutoReferencia.AsString := SQLProdutoPRODA60REFER.AsString;
          tblTempTranferenciaProdutoValorCusto.AsFloat := SQLProdutoPRODN3VLRCUSTO.Value;
          EdtCustoTotal.Value := EdtCustoTotal.Value + SQLProdutoPRODN3VLRCUSTO.Value;
          EdtQtdeTotalItens.Value := EdtQtdeTotalItens.Value + tblTempTranferenciaProdutoQtde.Value;
        ...
        fDlgTransferencia.mTransfPedido.Next;
      end;
    finally
      DestroyWindow;
    end;
...
procedure TFormTelaTransferencia.SpeedButton1Click(Sender: TObject);
var
  xSql: string;
begin
  inherited;
  if EditPedidoCompra.Text <> '' then
  begin
    xSql := 'select * from pedidocompraitem where pdcpa13id = ' + QuotedStr(EditPedidoCompra.Text);
    try
      if fDlgTransferencia = nil then
        fDlgTransferencia := TfDlgTransferencia.Create(nil);
      try
        dm.sqlConsulta.SQL.Clear;
        dm.sqlConsulta.SQL.Add(xSql);
        try
          dm.sqlConsulta.Open;
          dm.sqlConsulta.First;
          while not dm.sqlConsulta.eof do
          begin
            if not fDlgTransferencia.mTransfPedido.Active then
              fDlgTransferencia.mTransfPedido.Active := True;
            fDlgTransferencia.mTransfPedido.Append;
            fDlgTransferencia.mTransfPedidoQuantidade.AsFloat := dm.sqlConsulta.FieldByName('PCITN3QTDEMBAL').AsFloat * dm.sqlConsulta.FieldByName('PCITN3CAPEMBAL').AsFloat;
            fDlgTransferencia.mTransfPedidoQuantidade_Informada.AsFloat := dm.sqlConsulta.FieldByName('PCITN3QTDEMBAL').AsFloat * dm.sqlConsulta.FieldByName('PCITN3CAPEMBAL').AsFloat;
            fDlgTransferencia.mTransfPedidoCodigo_Produto.AsInteger := dm.sqlConsulta.FieldByName('PRODICOD').AsInteger;
            fDlgTransferencia.mTransfPedidoCodigo_Barra.AsString := SQLLocate('PRODUTO', 'PRODICOD', 'PRODA60CODBAR', dm.sqlConsulta.FieldByName('PRODICOD').AsString);
            fDlgTransferencia.mTransfPedidoNome_Produto.AsString := SQLLocate('PRODUTO', 'PRODICOD', 'PRODA60DESCR', dm.sqlConsulta.FieldByName('PRODICOD').AsString);
            fDlgTransferencia.mTransfPedidoReferencia.AsString := SQLLocate('PRODUTO', 'PRODICOD', 'PRODA60REFER', dm.sqlConsulta.FieldByName('PRODICOD').AsString);
            fDlgTransferencia.mTransfPedido.Post;
            dm.sqlConsulta.Next;
          end;
          fDlgTransferencia.ShowModal;
          if fDlgTransferencia.ModalResult = MrOK then
          begin
            RefazTabela(tblTempTranferencia, tblTempTranferencia.Name);
            incluiProdutoPedido;
            fDlgTransferencia.mTransfPedido.EmptyDataSet;
          end;
        except
          on e: Exception do
          begin
            MessageDlg('Problema ao Importar Pedido de compra' + #13 + e.message, mtWarning, [mbOK], 0);
          end;
        end;
      ...
      FreeAndNil(fDlgTransferencia);
      EditPedidoCompra.Clear;
    end;
  end;
