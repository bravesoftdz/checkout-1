...
uses
  ...
  AdvOfficeStatusBarStylers, AdvPanel, ppDBBDE;
...
uses  DataModulo, CadastroMotivoBloqueio,
     UnitLibrary, TelaConsultaTabelaPreco, CadastroConsulta,  CadastroCep,
     TelaClienteHistorico, TelaConsultaPlanoRecebimento, TelaNegociacaoDivida, WaitWindow, TelaDadosTeleEntrega,
     CadastroTipoCliente, TelaGeracaoMalaDireta, RelatorioExtratoCliente;
  ...
  if SQLTemplate.State in [DsInsert] then
    ...
    if SQLTemplate.FieldByName('CLIEA60RAZAOSOC').AsString <> '' then
      begin
        ...
        if IdCliente <> '' then
          begin
            if not Pergunta('NAO','O Sistema verificou que já existe no ID = '+IdCliente+' um Cliente com este Nome/Razão Social. Deseja Continuar Mesmo Assim ?') then
              Abort;
          end;
        IdCliente := '';
...
begin
  ...
  if not SQLTemplate.IsEmpty then
    begin
      while not SQLTemplate.eof do
        begin
          if Pergunta('Não','Deseja bloquear todos os clientes que estejam na lista de SPC pela cobrança?') then
            begin
...
{  ValorDebito := 0;
  ...
  if Historico = '' then
    begin
      Showmessage('É obrigado informar um motivo!');
      Exit;
    end;
  if ValorDebito <= 0 then
    begin
      Showmessage('É obrigado informar um valor maior que zero!');
      Exit;
    end;
