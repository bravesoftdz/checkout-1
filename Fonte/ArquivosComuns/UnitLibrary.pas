...
var
  ...
  OrdemGrupo, OrdemProduto, FiltraTerminal, Servidor_HostName, Servidor_Database, NroViasImpVenda, TabelaMaisTerminalDesc : String;
...
function  RetornaCodUF(Sigla:String):String;
...
function  CalculaTotalDevolucao : Double ;
...
function  RetornarGrupoTrabalho  : String;
procedure GravaMovimentoCaixa( SQLTotalizadorCaixa,
                               ...
                               MOVNORMREV,
                               Tipo : string) ;
procedure DesligarWindows ;
procedure AlterarPathAlias(NomeAlias, Path : string);
function  IsNumeric(Valor, Tipo : String) : boolean ; //- Criada em 29/12/2001 -\\
...
procedure EtiquetaImpTermica(NumCol:Integer; COM, TextZebra, Tipo, PathArqTXT:String) ;
//CHAMADA DE FUNCAO DE DLL
function  ExitWindowsEx(uFlags : integer; dwReserved : word) : boolean; external 'user32.dll' name 'ExitWindowsEx';
Function  TxStr(comm, buf : string;nbyte : Integer) : Integer;far; stdcall external 'SERIAL32.dll';
//////////////////////////
...
procedure CriaParcelasPagar(SQLParcelas : TDataSet; CodPlanoPagto : Integer; DataBaseFat : TDateTime; ValorNota : Double);
...
procedure AlteraPlanoDeContaNoMovBancario(IdContaReceber,ContaDebito,ContaCredito:String);
function  ConverteValorParaStringSemPontoSemVirgula(Valor:Double;Tamanho,CasasDecimais:Integer): String;
...
procedure GravaCustoMedioProdutoNotaCompra(NOCPA13ID, NROITEM : String; VlrCustoMedio : Currency);
procedure RefazTabelaTemp(Tabela: TTable; Abrir: Boolean);
function  TiraUltimoCaracter(Str: String; Caracter: Char): String;
...
function  TiraCaracteres(Texto:String): String;
//Fabiano
...
function Left(Str:String; Nro: Integer): String;
begin
  result := AllTrim(Copy(AllTrim(Str) + Spaces(Nro), 1, Nro));
end;
...
Var
  Temp: String;
begin
  Temp := Spaces(Nro) + AllTrim(Str);
  result := AllTrim(Copy(Temp, (Length(Temp) - Nro), Nro));
end;
...
//////////// IMPRESSAO DE ETIQUETAS EM ARQ TEXTO
procedure EtiquetaImpTermica(NumCol:Integer; COM, TextZebra, Tipo, PathArqTXT:String) ;
var
  TrCODIGOBAR,
  TrDESCRICAO,
  ...
  TrInfo01EtqBarras,
  TrInfo02EtqBarras,
  TrUNIDADE : String;//As que são trocadas''Tr'' (O registro no banco)
  ColCount,Fim,i,
  RespSerial,
  QuantLinhas      : Integer;
  TblEtiquetas     : TTable ;
  ListaZebra       : TStringList ;
  FileTXT          : TextFile;// Arquivos para a Balanca
  EndBloco,
  AbreArqTxt,
  Sair             : boolean ;
begin
 { * * * R E G R A S  P A R A R  P R O G R A M A Ç Ã O  * * *
 - DEVE-SE COLOCAR NAS POSIÇÕES ONDE DEVEM SER IMPRESSOS DADOS
   AS RESPECTIVAS PALAVRAS:
   CODREDUZ       - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    CODIGO REDUZIDO ENCONTRADO NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   DESCRICAO      - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    DESCRICAO(15 CHR) DO PRODUTO ENCONTRADA NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   DESCRAUX       - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA SEGUNDA PARTE(15 CHR)
                    DA DESCRICAO DO PRODUTO ENCONTRADA NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   DESC30CHR      - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA DESCRICAO REDUZIDA(30 CHR)
                    DA DESCRICAO DO PRODUTO ENCONTRADA NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   CODBAR         - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    CODIGO DE BARRAS ENCONTRADO NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   PRECO          - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    PREÇO ENCONTRADO NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   VAREJO          - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    PREÇO ENCONTRADO NO ARQUIVO MESMO QUE EXISTA PRECO PROMOCIONAL PRODN3PRECOVENDA
   PROMOCIONAL    - PREÇO PROMOCIONAL INCLUIDO NO SISTEMA
   ATACADO        - PRECO DE VENDA INCLUIDO NO CAMPO PRODN3PRECOVENDA2
   REFERENCIA     - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    REFERENCIA ENCONTRADA NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   MARCA          - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    MARCA ENCONTRADA NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   TAMANHO        - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    TAMANHO ENCONTRADO NO ARQUIVO
                    TEMPORÁRIO TblEtiquetas.DB;
   CORES          - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    COR ENCONTRADO NO ARQUIVO
   QUANTIDADE     - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    COR ENCONTRADO NO ARQUIVO;
                    TEMPORÁRIO TblEtiquetas.DB;
   LOTE           - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    COR ENCONTRADO NO ARQUIVO;
                    TEMPORÁRIO TblEtiquetas.DB;
   INFO01ETQBARRAS- ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    INFO01ETQBARRAS ENCONTRADO NO ARQUIVO;
                    TEMPORÁRIO TblEtiquetas.DB;
   INFO02ETQBARRAS- ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    INFO02ETQBARRAS ENCONTRADO NO ARQUIVO;
                    TEMPORÁRIO TblEtiquetas.DB;
   UNIDADE        - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELO
                    INFO02ETQBARRAS ENCONTRADO NO ARQUIVO;
                    TEMPORÁRIO TblEtiquetas.DB;
   ENDBLOCO       - ESTA PALAVRA SERÁ USADA PARA INFORMAR
                    O TERMINO DO BLOCO DE CONFIGURAÇÃO DE
                    DE UMA COLUNA DE IMPRESSÃO;
   DD/YY          - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    DATA CORRENTE USANDO ESTA STRING PARA
                    FORMATÁ-LA;
   MM/YY          - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    DATA CORRENTE USANDO ESTA STRING PARA
                    FORMATÁ-LA;
   DD/MM/YYYY     - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    DATA CORRENTE USANDO ESTA STRING PARA
                    FORMATÁ-LA;
   CHR(STX)       - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    FUNÇÃO CHR(2);
   CHR(27)        - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    FUNÇÃO CHR(27);
   CHR(13)        - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    FUNÇÃO CHR(13);
   ^B             - ONDE FOR ENCONTRADA ESTA PALAVRA NO
                    TEXTO O SISTEMA IRA SUBSTITUIR PELA
                    FUNÇÃO CHR(02);}

  TblEtiquetas                 := TTable.Create(Application) ;
  TblEtiquetas.DatabaseName    := 'EASY_Temp';
  TblEtiquetas.TableName       := 'Etiquetas.DB' ;
  TblEtiquetas.TableType       := ttParadox ;
//  TblEtiquetas.IndexFieldNames := 'Descricao' ;
  TblEtiquetas.Active          := True ;
  ListaZebra                   := TStringList.Create ;
  Sair                         := false ;
  AbreArqTxt                   := True ;

  TblEtiquetas.Last ;
  TrCODIGOREDUZIDO  := Format('%.6d', [TblEtiquetas.FieldByName('ProdutoCodigo').AsInteger]);
  TrCODIGOBAR       := TblEtiquetas.FieldByName('CodigoBarras').AsString ;
  TrDESCRICAO       := Copy(TblEtiquetas.FieldByName('Descricao').AsString,1,15) ;
  TrDESCRAUX        := Copy(TblEtiquetas.FieldByName('Descricao').AsString,16,15) ;
  TrDESC30CHR       := TblEtiquetas.FieldByName('DescricaoReduzida').AsString ;

  TrPRECO           := FormatFloat('###0.00',TblEtiquetas.fieldByName('Preco').asFloat);
  TrVAREJO          := FormatFloat('###0.00',TblEtiquetas.fieldByName('PRECOVAREJO').asFloat);
  TrATACADO         := FormatFloat('###0.00',TblEtiquetas.fieldByName('PRECOATACADO').asFloat);
  TrPROMOCIONAL     := FormatFloat('###0.00',TblEtiquetas.fieldByName('PRECOPROMO').asFloat);

  TrREFERENCIA      := TblEtiquetas.FieldByName('Referencia').AsString ;
  TrMARCA           := TblEtiquetas.FieldByName('MARCA').AsString ;
  if TblEtiquetas.FieldByName('Tamanho').AsString <> '' then
    TrTAMANHO       := TblEtiquetas.FieldByName('Tamanho').AsString
  else
    TrTAMANHO       := '**';
  TrCOR             := TblEtiquetas.FieldByName('Cor').AsString ;
  TrDTULTENTR       := TblEtiquetas.FieldByName('DtUltEntr').AsString ;
  TrNFNUMERO        := TblEtiquetas.FieldByName('NFNUMERO').AsString ;
  TrQUANTIDADE      := FormatFloat('0000',TblEtiquetas.FieldByName('Quant').AsInteger);
  TrNroLote         := TblEtiquetas.FieldByName('Lote').AsString;
  TrInfo01EtqBarras := TblEtiquetas.FieldByName('Info01EtqBarras').AsString;
  TrInfo02EtqBarras := TblEtiquetas.FieldByName('Info02EtqBarras').AsString;
  TrUNIDADE         := TblEtiquetas.FieldByName('Unidade').AsString;
  if (Tipo = 'ZEBRA_ZPL') then
    begin
      QuantLinhas := 1 ;
      Application.CreateForm(TFormTelaImpressaoTermicaCodBar, FormTelaImpressaoTermicaCodBar)
    end
  else
    QuantLinhas := TblEtiquetas.RecordCount ;

  while not Sair do
  begin
    ListaZebra.Text := TextZebra ;
    Fim             := 0;
    i               := 0;

    for ColCount := 1 to NumCol do
    begin
      EndBloco := false;
      while (not EndBloco) and (not TblEtiquetas.BOF) do
        begin
          if Pos('DD/YY', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('DD/YY',ListaZebra[i])-1) + FormatDateTime('DD/YY',Date) +Copy(ListaZebra[i],Pos('DD/YY',ListaZebra[i])+Length('DD/YY'),Length(ListaZebra[i])));
          if Pos('MM/YY', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('MM/YY',ListaZebra[i])-1) + FormatDateTime('MM/YY',Date) +Copy(ListaZebra[i],Pos('MM/YY',ListaZebra[i])+Length('MM/YY'),Length(ListaZebra[i])));
          if Pos('DD/MM/YYYY', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('DD/MM/YYYY',ListaZebra[i])-1) + FormatDateTime('DD/MM/YYYY',Date) +Copy(ListaZebra[i],Pos('DD/MM/YYYY',ListaZebra[i])+Length('DD/MM/YYYY'),Length(ListaZebra[i])));
          if Pos('CHR(STX)', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CHR(STX)',ListaZebra[i])-1) + Chr(2) +Copy(ListaZebra[i],Pos('CHR(STX)',ListaZebra[i])+Length('CHR(STX)'),Length(ListaZebra[i])));
          if Pos('CHR(27)', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CHR(27)',ListaZebra[i])-1) + CHR(27) +Copy(ListaZebra[i],Pos('CHR(27)',ListaZebra[i])+Length('CHR(27)'),Length(ListaZebra[i])));
          If Pos('CHR(13)', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CHR(13)',ListaZebra[i])-1) + CHR(13) +Copy(ListaZebra[i],Pos('CHR(13)',ListaZebra[i])+Length('CHR(13)'),Length(ListaZebra[i])));
          If Pos('CHR(12)', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CHR(12)',ListaZebra[i])-1) + CHR(12) +Copy(ListaZebra[i],Pos('CHR(12)',ListaZebra[i])+Length('CHR(12)'),Length(ListaZebra[i])));
          If Pos('CHR(10)', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CHR(10)',ListaZebra[i])-1) + CHR(10) +Copy(ListaZebra[i],Pos('CHR(10)',ListaZebra[i])+Length('CHR(10)'),Length(ListaZebra[i])));
          If Pos('^B', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('^B',ListaZebra[i])-1) + #02 +Copy(ListaZebra[i],Pos('^B',ListaZebra[i])+Length('^B'),Length(ListaZebra[i])));
          If Pos('CODREDUZ', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CODREDUZ',ListaZebra[i])-1) +TrCODIGOREDUZIDO +Copy(ListaZebra[i],Pos('CODREDUZ',ListaZebra[i])+Length('CODREDUZ'),Length(ListaZebra[i])));
          If Pos('DESCRICAO', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('DESCRICAO',ListaZebra[i])-1) +TrDESCRICAO + Copy(ListaZebra[i],Pos('DESCRICAO',ListaZebra[i])+Length('DESCRICAO'),Length(ListaZebra[i])));
          If Pos('DESCRAUX', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('DESCRAUX',ListaZebra[i])-1) +TrDESCRAUX + Copy(ListaZebra[i],Pos('DESCRAUX',ListaZebra[i])+Length('DESCRAUX'),Length(ListaZebra[i])));
          If Pos('QUANTIDADE', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('QUANTIDADE',ListaZebra[i])-1) + TrQUANTIDADE +Copy(ListaZebra[i],Pos('QUANTIDADE',ListaZebra[i])+Length('QUANTIDADE'),Length(ListaZebra[i])));
          If Pos('CODBAR', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CODBAR',ListaZebra[i])-1) +TrCODIGOBAR +Copy(ListaZebra[i],Pos('CODBAR',ListaZebra[i])+Length('CODBAR'),Length(ListaZebra[i])));


          {
          PRECO   = Recebe o valor de venda varejo caso não exista preco promocional, caso contrario recebe preco promocional
          VAREJO  = Recebe sempre o preco de venda varejo PRODN3VLRVENDA
          ATACADO = Recebe sempre o preco de venda atacado PRODN3VLRVENDA2
          PROMOCIONAL = Recebe sempre o preco promocional do sistema
          }

          If Pos('PRECO', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('PRECO',ListaZebra[i])-1) +TrPRECO +Copy(ListaZebra[i],Pos('PRECO',ListaZebra[i])+Length('PRECO'),Length(ListaZebra[i])));
          If Pos('VAREJO', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('VAREJO',ListaZebra[i])-1) +TrVAREJO +Copy(ListaZebra[i],Pos('VAREJO',ListaZebra[i])+Length('VAREJO'),Length(ListaZebra[i])));
          If Pos('ATACADO', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('ATACADO',ListaZebra[i])-1) +TrATACADO +Copy(ListaZebra[i],Pos('ATACADO',ListaZebra[i])+Length('ATACADO'),Length(ListaZebra[i])));
          If Pos('PROMOCIONAL', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('PROMOCIONAL',ListaZebra[i])-1) +TrPROMOCIONAL +Copy(ListaZebra[i],Pos('PROMOCIONAL',ListaZebra[i])+Length('PROMOCIONAL'),Length(ListaZebra[i])));


          If Pos('REFERENCIA', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('REFERENCIA',ListaZebra[i])-1) +TrREFERENCIA +Copy(ListaZebra[i],Pos('REFERENCIA',ListaZebra[i])+Length('REFERENCIA'),Length(ListaZebra[i])));
          If Pos('MARCA', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('MARCA',ListaZebra[i])-1) +TrMARCA +Copy(ListaZebra[i],Pos('MARCA',ListaZebra[i])+Length('MARCA'),Length(ListaZebra[i])));
          If Pos('TAMANHO', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('TAMANHO',ListaZebra[i])-1) +TrTAMANHO +Copy(ListaZebra[i],Pos('TAMANHO',ListaZebra[i])+Length('TAMANHO'),Length(ListaZebra[i])));
          If Pos('DTULTENTRA', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('DTULTENTRA',ListaZebra[i])-1) + TrDTULTENTR +Copy(ListaZebra[i],Pos('DTULTENTRA',ListaZebra[i])+Length('DTULTENTRA'),Length(ListaZebra[i])));
          If Pos('NFNUMERO', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('NFNUMERO',ListaZebra[i])-1) + TrNFNUMERO +Copy(ListaZebra[i],Pos('NFNUMERO',ListaZebra[i])+Length('NFNUMERO'),Length(ListaZebra[i])));
          If Pos('CORES', ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('CORES',ListaZebra[i])-1) + TrCOR +Copy(ListaZebra[i],Pos('CORES',ListaZebra[i])+Length('CORES'),Length(ListaZebra[i])));
          if Pos('NROLOTE',ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('NROLOTE',ListaZebra[i])-1) + TrNROLOTE +Copy(ListaZebra[i],Pos('NROLOTE',ListaZebra[i])+Length('NROLOTE'),Length(ListaZebra[i])));
          if Pos('INFO01ETQBARRAS',ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('INFO01ETQBARRAS',ListaZebra[i])-1) + TrInfo01EtqBarras +Copy(ListaZebra[i],Pos('INFO01ETQBARRAS',ListaZebra[i])+Length('INFO01ETQBARRAS'),Length(ListaZebra[i])));
          if Pos('INFO02ETQBARRAS',ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('INFO02ETQBARRAS',ListaZebra[i])-1) + TrInfo02EtqBarras +Copy(ListaZebra[i],Pos('INFO02ETQBARRAS',ListaZebra[i])+Length('INFO02ETQBARRAS'),Length(ListaZebra[i])));
          if Pos('UNIDADE',ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('UNIDADE',ListaZebra[i])-1) + TrInfo02EtqBarras +Copy(ListaZebra[i],Pos('UNIDADE',ListaZebra[i])+Length('UNIDADE'),Length(ListaZebra[i])));
          if Pos('DESC30CHR',ListaZebra[i]) > 0 then
            ListaZebra.Strings[i] := (Copy(ListaZebra[i], 0,Pos('DESC30CHR',ListaZebra[i])-1) + TrDESC30CHR +Copy(ListaZebra[i],Pos('DESC30CHR',ListaZebra[i])+Length('DESC30CHR'),Length(ListaZebra[i])));
          If Pos('ENDBLOCO', ListaZebra[i]) > 0 then
            ENDBLOCO := true;
          inc(i);
      end ;
        TblEtiquetas.Prior ;
        if not TblEtiquetas.BOF then
        begin
          TrCODIGOREDUZIDO := Format('%.6d', [TblEtiquetas.FieldByName('ProdutoCodigo').AsInteger]);
          TrCODIGOBAR      := TblEtiquetas.FieldByName('CodigoBarras').asstring;
          TrDESCRICAO      := Copy(TblEtiquetas.FieldByName('Descricao').AsString,1,15) ;
          TrDESCRAUX       := Copy(TblEtiquetas.FieldByName('Descricao').AsString,16,14);
          TrDESC30CHR      := TblEtiquetas.FieldByName('DescricaoReduzida').AsString ;
          ...
          if (Tipo = 'ZEBRA_ZPL') then
            ...
            QuantLinhas := TblEtiquetas.FieldByName('Quant').Value ;
        end
        else begin
          Sair := true;
          Fim  := i - 1;
    ...
    //Mandar Texto Para Impressora
    if COM = 'ARQ' then
       COM := InputBox('Impressão Código Barras', 'Digite o arquivo texto', PathArqTXT);

    if (Tipo = 'URAUSECBC') or
       (Tipo = 'ARQTEXTO') or
       (Tipo = 'RABBIT214') or
       (Tipo = 'ELTTPL2642')or
       (Tipo = 'ALLEGRO2') then
    begin
      ...
      if AbreArqTxt then
      begin
        AssignFile(FileTXT, COM) ;
        ReWrite(FileTXT) ;
      end ;

      if (Tipo = 'ARQTEXTO') or (Tipo = 'ZEBRA_ZPL') then
        AbreArqTxt := false ;

      if Fim = 0 then
        Fim := ListaZebra.Count-1 ;

      for i := 0 To Fim do
        if Pos('ENDBLOCO', ListaZebra[i]) = 0 then
          Writeln(FileTXT, ListaZebra[i]);
      if Tipo = 'URAUSECBC' then
        Writeln(FileTXT, Chr(12))
      else
        if (Fim <> ListaZebra.Count-1) then
          Writeln(FileTXT, '^XZ');
    ...
    begin
      ...
      for i := 0 To Fim do
        if Pos('ENDBLOCO', ListaZebra[i]) = 0 then
        begin
          FormTelaImpressaoTermicaCodBar.MemTable.Append ;
          FormTelaImpressaoTermicaCodBar.MemTableLinha.Value := ListaZebra[i] ;
          FormTelaImpressaoTermicaCodBar.MemTable.Post ;
           //FormTelaImpressaoTermicaCodBar.Linhas.Lines.Add(ListaZebra[i]) ;
        end ;

      if (Fim <> ListaZebra.Count-1) then
      begin
        FormTelaImpressaoTermicaCodBar.MemTable.Append ;
        FormTelaImpressaoTermicaCodBar.MemTableLinha.Value := '^XZ' ;
        FormTelaImpressaoTermicaCodBar.MemTable.Post ;
        //FormTelaImpressaoTermicaCodBar.Linhas.Lines.Add('^XZ');
      end ;
    end ;

    if (Tipo = 'ALLEGRO2') then
    begin
      if Fim = 0 then
        Fim := ListaZebra.Count-1 ;
      ...
      for I := 0 To Fim do
        ...
        if Pos('ENDBLOCO', ListaZebra[i]) = 0 then
          RespSerial := TxStr(COM, PChar(ListaZebra[i]), Length(ListaZebra[i]));
    end ;
  end;
  ...
  if (Tipo = 'ZEBRA_ZPL') then
    FormTelaImpressaoTermicaCodBar.ppReport1.Print ;
...
procedure GravaMovimentoEstoque(SqlProd,
                                ...
                                NumDocOrig,
                                Lote : String) ;
procedure GravaMovimento(Produto,ProdutoFilho : Integer; Qtde : Double);
var
  ProxCod : integer ;
  SairMov : Boolean;
  QueryProduto : TQuery;
...
begin
  ...
  if not SqlProd.EOF then
    Begin
          ...
          while not SQLProdFilho.EOF do
            begin
              ...
              while not Sair do
                begin
                  try
                    GravaMovimento(SQLProdFilho.FindField('PRODICODFILHO').AsInteger,0,SQLProdFilho.FindField('PRODN3QTDE').AsFloat);
                    Sair := true;
                    Application.ProcessMessages;
                  except
                    Sair := False;
      ...
      // EFETUA A MOV DE ESTOQUE DO PRODUTO QUE É PAI DA COMPOSICAO
            ...
            begin
              while not SQLProdFilho.Eof do
                begin
                  ...
                  begin
                    try
                      GravaMovimento(SQLProdFilho.FieldByName('PRODICOD').AsInteger,SQLProdFilho.FieldByName('PRODICODFILHO').AsInteger,0);
                      Sair := True ;
                      Application.ProcessMessages ;
                    except
                      Sair := False;
...
begin
  ...
  DM.SQLTemplate.SQL.Add('from ITENSDEVOLUCAOTEMP') ;
  ...
  if DM.SQLTemplate.FieldByName('TotalRetorno').Value > 0 then
    Result := DM.SQLTemplate.FieldByName('TotalRetorno').Value
  else
    Result := 0 ;
end ;
...
begin
  if TipoPadrao = 'DIN' then
    HistoricoPadraoCaixa := 'DINHEIRO(+)' ;
  if TipoPadrao = 'CHQ' then
    HistoricoPadraoCaixa := 'CHEQUE(+)' ;
  if TipoPadrao = 'CRD' then
    HistoricoPadraoCaixa := 'CREDIÁRIO' ;
  if TipoPadrao = 'CNV' then
    HistoricoPadraoCaixa := 'CONVÊNIO' ;
  if TipoPadrao = 'CRT' then
    HistoricoPadraoCaixa := 'CARTÃO' ;
  if TipoPadrao = 'RTC' then
    HistoricoPadraoCaixa := 'RETORNO CONSIGNAÇÃO' ;
end ;
...
begin
  ...
  If (Not PrecoOk) AND (TabelaPrecoCliente <> '') Then
    if (TabelaPrecoCliente <> 'V') and (copy(TabelaPrecoCliente,1,1) <> 'A') then
      Begin
        DM.SQLPreco.ParamByName('TPRCICOD').asString  := TabelaPrecoCliente;
        DM.SQLPreco.ParamByName('PRODICOD').asInteger := QueryProduto.FindField('PRODICOD').asInteger;
        DM.SQLPreco.Open;
        If Not DM.SQLPreco.Eof Then
          Begin
            if (DM.SQLPreco.FieldByName('TPCPDINIPROMO').Value <> null) and
               (DM.SQLPreco.FieldByName('TPCPDFIMPROMO').Value <> null) and
               (Date >= DM.SQLPreco.FieldByName('TPCPDINIPROMO').asDateTime) and
               (Date <= DM.SQLPreco.FieldByName('TPCPDFIMPROMO').asDateTime) then
              RetornaPreco := DM.SQLPreco.FieldByName('TPCPN3VLRVENDAPROM').Value
            else
              RetornaPreco := DM.SQLPreco.FieldByName('TPCPN3VLRVENDA').Value;
            PrecoOk := True;
          End;
        DM.SQLPreco.Close;
      End;
  ...
  If (Not PrecoOk) AND (TabelaPrecoEmpresa<>'') Then
    Begin
      DM.SQLPreco.ParamByName('TPRCICOD').asString  := TabelaPrecoEmpresa;
      DM.SQLPreco.ParamByName('PRODICOD').asInteger := QueryProduto.FindField('PRODICOD').asInteger;
      DM.SQLPreco.Open;
      If Not DM.SQLPreco.Eof Then
        Begin
...
begin
  ...
  try
    ...
    Registro.Openkey('HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\VxD\VNETSUP',false) ;
    Result := Registro.Readstring('WorkGroup') ;
    if Result = '' then
      begin
        IniFile := TIniFile.Create(DM.PathAplicacao + 'Terminal.ini');
        Result  := IniFile.ReadString(AnsiUpperCase('WORKGROUP'),AnsiUpperCase('Name'),Grupo);
      end;
  except
    informa('Não foi possível encontrar o grupo de trabalho do computador! Por favor entre em contato com o supote!');
  end;
end;
...
begin
  ...
  begin
    Informa('Alias => '+NomeAlias+' não existe! Operação Cancelada!');
    Exit;
  end;
  ...
  try
    ...
    Session.GetAliasParams(NomeAlias, AParams);
    AParams.Clear ;
    AParams.Add('SERVER NAME=' + Path) ;
    Session.ModifyAlias(NomeAlias, AParams) ;
    ...
    begin
      if SQLOperacao.FindField('OPBCCALTSALDO').AsString[1] = 'S' then
        begin
          ...
          IDMovBanco := IDMovBanco + FormatFloat('000',TerminalAtual);
          IDMovBanco := IDMovBanco + FormatFloat('000000',ProximoCodigo);
...
procedure CriaParcelasPagar(SQLParcelas : TDataSet; CodPlanoPagto : Integer; DataBaseFat : TDateTime; ValorNota : Double);
var
  SQLPlano, SQLPlanoParcelas : TRxQuery;
begin
// O OBJETIVO DA PROCEDURE É SOMENTE GERAR AS INFORMACOES DAS PARCELAS
// NÃO ALMIMENTANDO CAMPOS AUTO INCREMENTO OU IDS, NEM MESMO OS CAMPOS
// DE RELACIONAMENTO, APENAS DATA, VENCIMENTO, VALOR E PARCELA SAO ALIMENTADOS;

  if CodPlanoPagto > 0 then
    begin
      SQLPlano := TRxQuery.Create(DM);
      SQLPlano.DatabaseName := 'DB';
      with SQLPlano do
        begin
          SQL.Clear;
          SQL.ADD('SELECT * FROM PLANOPAGAMENTO WHERE PLPGICOD = ' + IntToStr(CodPlanoPagto));
          Open;
          if IsEmpty then
            begin
              Informa('Plano de Pagamento não encontrado, favor verificar!');
              Exit;
            end
          else
            begin
              SQLPlanoParcelas := TRxQuery.Create(DM);
              SQLPlanoParcelas.DatabaseName := 'DB';
              with SQLPlanoParcelas do
                begin
                  SQL.Clear;
                  SQL.Add('SELECT * FROM PLANOPAGAMENTOPARCELA WHERE PLPGICOD = ' + IntToStr(CodPlanoPagto));
                  SQL.Add(' ORDER BY PLPPINROPARC');
                  Open;
                  if IsEmpty then
                    begin
                      Informa('O Plano de Pagamento Informado não possui parcelas, favor verificar!');
                      Exit;
                    end
                  else
                    begin
                      SQLPlanoParcelas.First;
                      while not SQLPlanoParcelas.EOF do
                        begin
                          SQLParcelas.Append;
                          SQLParcelas.FieldByName('CTPGINROPARC').AsInteger := SQLPlanoParcelas.FieldByName('PLPPINROPARC').AsInteger;
                          SQLParcelas.FieldByName('CTPGDTEMIS').AsDateTime  := DataBaseFat;
                          SQLParcelas.FieldByName('CTPGDVENC').AsDateTime   := IncDay(DataBaseFat,SQLPlanoParcelas.FieldByName('PLPPINRODIAS').AsInteger);
                          SQLParcelas.FieldByName('CTPGN3VLR').AsFloat      := ValorNota / SQLPlanoParcelas.RecordCount;
                          SQLParcelas.Post;
                          SQLPlanoParcelas.Next;
                        end;
                    end;
                    SQLPlanoParcelas.Free;
                end;
            end;
            SQLPlano.Free;
        end;
    ...
    begin
      Informa('Plano de Pagamento Inválido!');
      Exit;
    end;
end;
...
begin
      ...
      begin
        ...
        if ContaDebito <> '' then
          begin
            Query.Close;
            Query.SQL.Clear;
            Query.SQL.ADD('UPDATE MOVIMENTOBANCO SET Pendente=''S'', PLCTA15COD = ''' + ContaDebito + '''');
            Query.SQL.ADD(' WHERE CTRCA13ID = ''' + IdContaReceber + '''');
            Query.ExecSQL;
          end;
        Query.Close;
  ...
  EXCEPT
   ON E:EXCEPTION DO
     BEGIN
       Informa('Erro ao Trocar Plano de Contas no Mov. Bancário. ANOTE O ERRO: ' + E.Message);
       Query.Destroy;
       Abort;
     END;
  END;
...
begin
  ...
  if CasasDecimais = 3 then
    ValorAux := FormatFloat('####0.000', Valor);
  if Pos(',', ValorAux) > 0 then
    Delete(ValorAux, Pos(',', ValorAux), 1);
  if Pos('.',ValorAux) > 0 then
    Delete(ValorAux, Pos('.', ValorAux), 1);

  Dif := Tamanho - Length(ValorAux) ;
  for i := 1 to Dif do
  begin
    ValorAux := '0' + ValorAux;
  End ;
  ConverteValorParaStringSemPontoSemVirgula := ValorAux;
end;
...
SQLAtualiza : TQuery;
begin
  if (NOCPA13ID = '') or (NROITEM = '') then
    Exit;

  SQLAtualiza := TQuery.Create(DM);
  SQLAtualiza.DatabaseName := 'DB';
  SQLAtualiza.Close;
  SQLAtualiza.SQL.Clear;
  SQLAtualiza.SQL.Add('UPDATE NOTACOMPRAITEM SET Pendente=''S'', NOCIN3VLRCUSTOMED = ' + ConvFloatToStr(VlrCustoMedio));
  SQLAtualiza.SQL.Add(' WHERE ');
  SQLAtualiza.SQL.Add('NOCPA13ID = ''' + NOCPA13ID + '''');
  SQLAtualiza.SQL.Add(' AND NOCIIITEM = ' + NROITEM);
  try
    SQLAtualiza.Prepare;
    SQLAtualiza.ExecSQL;
  except
    on E:Exception do
      begin
        Informa('Problemas ao gravar custo médio do produto da nota de compra, ANOTE O ERRO: ' + #13 + E.Message);
        SQLAtualiza.Close;
        SQLAtualiza.Destroy;
        Abort;
      end;
...
Begin
  Result  := '';
  if Sigla ='AC' Then Result := '12';
  if Sigla ='AL' Then Result := '27';
  if Sigla ='AM' Then Result := '13';
  if Sigla ='AP' Then Result := '16';
  if Sigla ='BA' Then Result := '29';
  if Sigla ='CE' Then Result := '23';
  if Sigla ='DF' Then Result := '53';
  if Sigla ='ES' Then Result := '32';
  if Sigla ='GO' Then Result := '52';
  if Sigla ='MA' Then Result := '21';
  if Sigla ='MG' Then Result := '31';
  if Sigla ='MS' Then Result := '50';
  if Sigla ='PA' Then Result := '15';
  if Sigla ='PB' Then Result := '25';
  if Sigla ='PE' Then Result := '26';
  if Sigla ='PI' Then Result := '22';
  if Sigla ='PR' Then Result := '41';
  if Sigla ='MT' Then Result := '51';
  if Sigla ='RJ' Then Result := '33';
  if Sigla ='RN' Then Result := '24';
  if Sigla ='RO' Then Result := '11';
  if Sigla ='RR' Then Result := '14';
  if Sigla ='RS' Then Result := '43';
  if Sigla ='SC' Then Result := '42';
  if Sigla ='SE' Then Result := '28';
  if Sigla ='SP' Then Result := '35';
  if Sigla ='TO' Then Result := '13';
  if Sigla ='EX' Then Result := '99';
End;
