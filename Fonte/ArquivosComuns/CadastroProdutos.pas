...
uses DataModulo,
     //CadastroSubgrupo, CadastroMarca,
     //CadastroGrupo, CadastroGrade, CadastroCor, CadastroTamanho,
     //CadastroUnidade, CadastroICMS, CadastroClassificacaoFiscal,
     //CadastroProdutoFornecedor, TelaCriacaoGrade,
     UnitLibrary,
     //CadastroProdutoComposicao,
     WaitWindow, WindowsLibrary,
     //CadastroProdutoSerie, CadastroProdutoTabelaPreco,
     DataModuloTemplate, //CadastroColecao, CadastroDecreto,
  TelaEntradaRapidaEstoque, TelaFotoExpandida, //CadastroBarras,
  TelaSaidaRapidaEstoque
  //CadastroNCM, CadastroTabCest
  ;
...
begin
 ...
 { LBPis_Cofins_OutrasDesp.Caption := '  Pis :   '  + Dm.SQLConfigVenda.FieldByName('CFVEN2PERCPIS').AsString    +
  ...
  // Carrega opções da Sit. Tributaria cfe. Regime Tributario da Empresa
...
begin
 inherited;
 // consistencia de dados para emissão de NFe
 Faltas:= '';
          ...
          if SQLGradeTamanho.RecordCount > 15 then
            begin
              Informa('Problemas no cadastro da grade... ' + #13 +
                      'Número de tamanhos excedeu o limite de 15, verifique seu cadastro!' + #13 +
...
begin
  ...
  if ValorVenda <>  SQLGradeProdutoPRODN3VLRVENDA.AsFloat then
    ...
    if (DM.SQLConfigGeralCFGECALCPRECOAUTOM.AsString <> 'N') and (DM.SQLConfigCompraCFCOCTOTPRCVENPROD.Value='U') and (SQLGradeProdutoPRODN3PERCMARGLUCR.AsFloat>0) then
...
begin
  ...
  if Pergunta('NAO','Voce apagará essa Cor e Tamanho! Continuar') then
    begin
      DM.DB.StartTransaction;
...
begin
  ...
  if AutenticaUsuario(UsuarioAtualNome,'USUACUSERMASTER',Retorno) = 'S' then
    ...
    begin
      Informa('Este alteração só poder ser feita por um usuário Master do sistema!');
      Abort;
    end;
end;
...
begin
    ...
    begin
      ...
      if SQLMovimentoEstoqueNOFIA13ID.AsString <> '' then
        begin
          ...
          // se o retorno for diferente de '' é cliente caso contrario é fornecedor
          Try
      ...
      if SQLMovimentoEstoqueMOVDA13ID.AsString <> '' then
        // se tiver OS- no campo MOVDA13ID é porque o sistema registrou uma ordem de serviço de mecanica.
  ...
  Report.ReportTitle       := 'Relatório de Movimentação de Produto';
  Report.WindowStyle.Title := 'Relatório de Movimentação de Produto';
  Report.Formulas.Retrieve ;
...
begin
  ...
  if SQLTemplatePRODCSERVICO.asstring = 'S' then
    SQLTemplatePRODA2TIPOITEM.AsString := '09' // serviço
  Else
...
begin
  inherited;
  if not Pergunta('NAO','Recalcular os Custos pela Composição dos produtos que estão sendo visualizados?') then
    Exit;
