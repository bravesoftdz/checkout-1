...
uses DataModulo, RXCtrls, Forms, RxQuery,  UnitLibrary, SysUtils, Dialogs ;
  ...
  function  AbrirCupomFiscal_Elgin_FIT(Var NroCupom : String) : boolean ;
  ...
  function  FecharCupomFiscal_Elgin_FIT(TipoAcrescDesc : String; Valor, VlrAcrescDesc, VlrTroco : Double ; Ecf_CNFV, Msg : String; SQLFPVista, SQLFPPrazo : TRxQuery;NomeCli,EnderecoCli,CidadeCli,DocumentoCli, IDCupom:string) : boolean ;
  ...
  function  Elgin_FIT_AbreRecebimentoNaoFiscal( Indice_do_Totalizador: String; Acrescimo_ou_Desconto: String; Tipo_Acrescimo_ou_Desconto: String; Valor_Acrescimo_ou_Desconto: String; Valor_do_Recebimento: String; Texto_Livre: String ): Integer; StdCall; External 'Daruma32.dll';
...
begin
  ...
  while not SQLFPVista.Eof do
    begin
      ...
      if i = 1 then
        EnviaFP_Elgin_FIT(NumerarioDescricao, StrToFloat(FormatFloat('##0.00',SQLFPVista.Fieldbyname('VALORPARC').AsFloat))+ VlrTroco)
      else
...
//  IntRet := Elgin_RecebimentoNaoFiscal(Pchar(Trim(Identificador)),Pchar(Trim('D')), Pchar(Trim('$')),Pchar(Trim('0,00')), Pchar(Trim(Vlr)), Pchar(Trim(Msg)));
  if IntRet > 0 then
...
function ImprimeRespostaCartao_Elgin_FIT(CaminhoRespostaCartao, CartaoDescricao, TravaTeclado : String) : boolean;
begin
{  IntRet := Elgin_TEF_ImprimirResposta(CaminhoRespostaCartao,CartaoDescricao,'1');
...
begin
  ...
  Vlr := FormatFloat('##0.00', Valor);
  IntRet := Elgin_Suprimento(Vlr, NumerarioDescricao) ;
  if IntRet > 0 then
