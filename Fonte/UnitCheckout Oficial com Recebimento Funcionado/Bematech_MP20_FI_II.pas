...
uses RXCtrls, RxQuery, Forms, DBTables, SysUtils, Dialogs ;
  ...
  function  FecharCupomFiscal_Bematech_MP20_FI_II(TipoAcrescDesc : String; Valor, VlrAcrescDesc, VlrTroco : Double ; Ecf_CNFV, Msg : String; SQLFPVista, SQLFPPrazo : TRxQuery;NomeCli,EnderecoCli,CidadeCli,DocumentoCli, IDCupom:string) : boolean ;
...
begin
 ...
 { if Application.FindComponent('DM') <> nil then
  ...
  begin
    Envia_FP_Bematech_MP20_FI_II(WSQL.FieldByName('NUMEA30DESCR').Value);
    WSQL.Next;
  end;  }
  ...
  SQLFPVista.First;
  i := 0; // Colocado por Adilson para imprimir Troco
  if not EnviouNumerariosECF then
    begin
      ...
      begin
        if I = 0 then
          begin
            if not EfetuaFPCupom_Bematech_MP20_FI_II(RetornaTotalizadorNumerarioECF(Ecf_ID, SQLFPVista.Fieldbyname('NUMEICOD').AsString),
                                                     SQLFPVista.Fieldbyname('VALORPARC').AsFloat + VlrTroco) then
              begin
                FecharCupomFiscal_Bematech_MP20_FI_II := false;
                Exit;
              end;
        ...
        SQLFPVista.Next ;
        // Dec(I); Removido por Adilson, nao entendi pq usa esse I, entao saquei ele fora
      end ;
