...
function FecharCupomFiscal_Bematech_MP25_FI(TipoAcrescDesc : String; Valor, VlrAcrescDesc, VlrTroco : Double ; Ecf_CNFV, Msg : String; SQLFPVista, SQLFPPrazo : TRxQuery;NomeCli,EnderecoCli,CidadeCli,DocumentoCli, IDCupom :string) : boolean ;
...
function ImprimeItem_Bematech_MP25_FI(Codigo: String; Descricao: String; Aliquota: String; Tipo_de_Quantidade: String; Quantidade: Double; Casas_Decimais: Integer; Valor_Unitario: Double; Tipo_de_Desconto: String; Valor_do_Desconto: Double ): boolean ;
var Valor_Unitario_ECF, Valor_Desconto_ECF, Quantidade_ECF, FormatSTRValor : String;
var TotalItemECF, TotalItemCalc : Double;
begin
  if dm.SQLConfigVendaCFVEINROCASASDEC.Value = 2 then FormatSTRValor := '##0.00';
  if dm.SQLConfigVendaCFVEINROCASASDEC.Value = 3 then FormatSTRValor := '##0.000';
  if Frac(Quantidade) > 0 then
    begin
      if FileExists('C:\Unit\Gestao\Truncada.txt') then
        begin
          TotalItemECF    := StrToFloat(FormatFloat(FormatSTRValor, StrToFloat(FormatFloat(FormatSTRValor,Valor_Unitario))*
      ...
      Quantidade_ECF     := FormatFloat('###0.000',Quantidade);
      Valor_Desconto_ECF := FormatFloat('##0.00',Valor_do_Desconto);
      if Bematech_FI_VendeItem(copy(Codigo,1,13),copy(Descricao,1,29),Aliquota,Tipo_de_Quantidade,Quantidade_ECF,dm.SQLConfigVendaCFVEINROCASASDEC.Value,Valor_Unitario_ECF,Tipo_de_Desconto,Valor_Desconto_ECF) = 1 then
        Result := True
      else
        Result := False;
    end
    ...
    begin
      ...
      Valor_Unitario_ECF := FormatFloat(FormatSTRValor,Valor_Unitario);
      Quantidade_ECF     := FormatFloat('###0',Quantidade);
...
function LinhaTextoLivre_Bematech_MP25_FI(Texto : string) : boolean ;
begin
  if Bematech_FI_UsaComprovanteNaoFiscalVinculado(Texto) = 1 then
...
begin
  ...
  while not SQLFPVista.Eof do
    begin
      ...
      if i = 1 then
        begin
          VlrRectoECF := SQLFPVista.Fieldbyname('VALORPARC').AsFloat+ VlrTroco;
          EnviaFP_Bematech_MP25_FI(NumerarioDescricao, VlrRectoECF );
        end
      else
