                ...
                begin
                  ...
                  End Else
                    begin
                      ...
                      {Alimenta as tabelas Sped c420, c460, c470 e c490 para depois apenas passalas no txt}
      ...
      while not zPesquisa.eof do
        begin
          ...
          begin
            ...
            //zPesquisa1.ParamByName('ID').AsString := zPesquisa.fieldbyname('CUPOA13ID').AsString;
            zPesquisa1.Open;
                      ...
                      if (zPesquisa1.fieldbyname('coitn2icmsaliq').Value > 0) then
                        begin
                          ...
                          If zPesquisa1.FieldByName('prodcservico').AsString = 'S' Then
                            ...
                            begin
                              // Adilson Base := ((zPesquisa1.fieldbyname('CPITN3VLRUNIT').Value * nQtde)-zPesquisa1.fieldbyname('CPITN2DESC').Value * (PercDescTotalRateado/100));
                              Base            := nVl_Item;
                              Cfop            := '5102';
                              Cst_Icms        := '000';
                        ...
                        begin
                          If zPesquisa1.FieldByName('PRODA1TIPO').AsString = '4' Then // Combustivies
                            ...
                            if ((Cst_Icms = '010') or (Cst_Icms = '060')) then
                              Cfop := '5405'
                            Else
                              Cfop := '5102';
      ...
      begin
        ...
        begin
          ...
          Linha :=  '|H010|' ;                                                                       // 01-Registro H010
                    if CkBarras.Checked then
                      Linha := Linha + zINVENTARIO.FieldByName('PRODA60CODBAR').AsString              + '|' // 02 COD_ITEM Código do item (campo 02 do Registro 0200)
                    else
                      Linha := Linha + zInventario.FieldByName('PRODICOD').AsString                   + '|' ;
                    Linha := Linha +
      ...
      begin
        Try
          ...
          if (z0150.FieldByName('CPF').AsString = '') and (z0150.FieldByName('CNPJ').AsString = '') then
            begin
              if zPesquisa.FieldByName('NOFIA13ID').AsString = '' then
                EXIT;
              z0150.Cancel;
...
Var ForPag, StatusNF, Chave, xCOD_SIT, CnpjParticipante: String;
...
Begin
  ...
  zPesquisa.SQL.Text := 'Select N.* from CUPOM N ' +
                        ...
                        'N.EMPRICOD='+ComboEmpresa.Value+' AND N.CUPOCSTATUS = ''A'' AND N.CHAVEACESSO is not null' ;
  zPesquisa.Open;
...

  {
    ...
    ((RetornaModeloNF(zPesquisa.FieldByName('SERIA5COD').AsString) = '01') or
     (RetornaModeloNF(zPesquisa.FieldByName('SERIA5COD').AsString) = '04'))
    then // Encerada - '1' = Nota Fiscal de Saida
      begin
        Exit; {Adilson removi pq parei de testar o modelo}
    ...
    begin
      ...
      While Not zPesquisa1.Eof do
        begin
          ...
          // Adilson Removido  if zPesquisa2.FieldByName('NOCIA3CSTICMS').AsInteger  in ([10, 30, 70]) then
            if (zPesquisa1.FieldByName('NOCIA3CSTICMS').AsInteger in ([10, 30, 40, 41, 50, 51, 60, 70, 90]))
            or (zPesquisa1.FieldByName('NOCIA3CSTICMS').AsInteger in ([130, 140, 141, 150, 151, 160]))
            or (zPesquisa1.FieldByName('NOCIA3CSTICMS').AsInteger-200 in ([10, 30, 40, 41, 50, 51, 60, 70, 90])) then
            Begin
