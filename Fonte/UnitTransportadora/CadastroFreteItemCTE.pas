...
type
  TFormCadastroFreteItemCTE = class(TFormCadastroTEMPLATE)
    QueryProduto: TQuery;
    QueryUnidade: TQuery;
    QueryIcms: TQuery;
...
var
  FormCadastroFreteItemCTE: TFormCadastroFreteItemCTE;
...
procedure TFormCadastroFreteItemCTE.FormCreate(Sender: TObject);
begin
  inherited;
  Tabela := 'FRETEINFO';
end;

procedure TFormCadastroFreteItemCTE.SQLTemplateNewRecord(DataSet: TDataSet);
begin
  inherited;
...
procedure TFormCadastroFreteItemCTE.SQLTemplateBeforeDelete(
  DataSet: TDataSet);
var
...
procedure TFormCadastroFreteItemCTE.SQLTemplateBeforeEdit(DataSet: TDataSet);
begin
  inherited;
  if DSMasterTemplate.DataSet.FieldByName('FRETCSTATUS').AsString <> 'A' then
    Abort
...
procedure TFormCadastroFreteItemCTE.SQLTemplateBeforeInsert(
  DataSet: TDataSet);
begin
  inherited;
  if DSMasterTemplate.DataSet.FieldByName('FRETCSTATUS').AsString <> 'A' then
...
procedure TFormCadastroFreteItemCTE.RxDBComboBox9Change(Sender: TObject);
begin
  inherited;
