...
begin
  ...
  // IMP.DADOS BASICOS DA VENDA
  ecfImpLinha(Pchar(Preenche('CUPOM(ID).: ' + IDCupom,'',48,2)));
  ecfImpLinha(Pchar(Preenche('VENDEDOR..: ' + ConverteAcentos((Application.FindComponent('FormTelaFechamentoVenda').FindComponent('LblNomeVendedor') as TRxLabel).Caption),'',48,2)));
  ecfImpLinha(Pchar(Preenche('PLANO.....: ' + ConverteAcentos((Application.FindComponent('FormTelaFechamentoVenda').FindComponent('LblNomePlano') as TRxLabel).Caption),'',48,2)));
  ecfImpLinha(Pchar(Preenche('CLIENTE...: ' + ConverteAcentos(NomeCli),'',48,2)));

  //IMPRIMIR DADOS DO CLIENTE
  if (Application.FindComponent('FormTelaFechamentoVenda') <> nil) and
     (NomeCli <> SQLLocate('CLIENTE','CLIEA13ID','CLIEA60RAZAOSOC','"'+DM.SQLTerminalAtivoCLIEA13ID.AsString+'"')) then
  if (ImprimeDadosClienteCupom = 'S') then
    begin
      ecfImpLinha(Pchar(Preenche('DOCUMENTO.: ' + ConverteAcentos(DocumentoCli),'',48,2)));
      ecfImpLinha(Pchar(Preenche('END.:' + ConverteAcentos(Copy(EnderecoCli, 01, 35)),'',48,2)));
      ecfImpLinha(Pchar(Preenche('CIDADE....: ' + ConverteAcentos(Copy(CidadeCli,   01, 28)),'',48,2)));
    end;

  ecfImpLinha(Pchar(Preenche(ConverteAcentos(Copy(OBSImpressaoCupom,01, 28)),'',48,2)));
