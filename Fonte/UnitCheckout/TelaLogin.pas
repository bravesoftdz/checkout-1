...
uses
  ...
  CurrEdit, dxGDIPlusClasses, AdvOfficeStatusBar, AdvOfficeStatusBarStylers,
  ComCtrls, TFlatPanelUnit;
type
...
begin
  ...
  if dm.SQLUsuarioEMPRESA_ACESSO.AsString <> '' then
    begin
      if (SQLTerminalEMPRICOD.Value <> dm.SQLUsuarioEMPRESA_ACESSO.Value) then
        begin
          Informa('Esse usuário não pode acessar esse terminal!');
          ComboPDV.SetFocus;
          Linha := 'c:\easy2solutions\log_usuario_'+dm.SQLUsuarioUSUAA60LOGIN.Value+' tentou acessar o terminal '+SQLTerminalTERMA60DESCR.Value+'.txt';
  ...
  begin
    ...
    VelocECFAtual := DM.SQLTerminalAtivoECF_VELOC.Value;
...
begin
  ...
  rxDataHora.caption := FormatDateTime('dd/mm/yyyy hh:mm', now);
