...
type
  TFormTelaFechamentoVenda = class(TForm)
    ...
    SQLConvenioCONVN2DESC: TFloatField;
    SQLConvenioCONVN2TAXA: TFloatField;
...
//            EntradaDadosKeyDown(EntradaDados,Tecla,[ssCtrl]);
                            ...
                            begin
                              ...
                              if (ECFAtual <> 'SIGTRON FS300') then
                                begin
                                  if (ECFAtual <> 'SIGTRON FS300') then
                                    begin
                                      if (DM.SQLTerminalAtivoTERMCECFIMPRCONFDIVIMPITENS.AsString = 'S') then
                                        begin
                                      ...
                                      if DM.SQLTerminalAtivoTERMCECFIMPRCONFDIVIMPFAT.AsString = 'S' then
                                        begin
        ...
        if (TerminalModo = 'C') and (not E_Orcamento)  then
          begin
            ...
            if (DM.SQLTerminalAtivoTERMCINFDADOCARTAO.Value = 'S') and (TipoPadrao = 'CRT') then
            begin
...
//                            FormTelaItens.TotalDescItens+VlrDesc,//WMOVICAIXN2VLRDEC
  ...
  while not SQLParcelasPrazoVendaTemp.Eof do
  begin
    TipoPadrao1 := SQLParcelasPrazoVendaTempTIPOPADR.AsString;
    ...
    if Copy(TipoPadrao1,1,3) = 'CHQ' then
      begin
        if TipoPadrao1 = 'CHQP' then
          ...
          Sigla   := 'VDVIS';
        MsgErro := 'Não há nenhuma Operação de Caixa configurada com a sigla de Venda Cheque!' ;
        VlrLanc := SQLParcelasPrazoVendaTempVALORVENCTO.Value;
      end ;
