...
uses
  ...
  OleCtrls, SHDocVw, ACBrNFe, pcnConversao, ACBrUtil, ACBrNFeDANFEClass, ACBrNFeDANFeESCPOS,
...
begin
  {$IFDEF ACBrNFeOpenSSL}
    ACBrNFe.Configuracoes.Certificados.ArquivoPFX  := dm.sqlEmpresa.FieldByName('EMPRA100CERTIFSERIE').AsString;
  ...
  ACBrNFe.Configuracoes.Geral.IdCSC  := dm.sqlEmpresa.FieldByName('idTOKEN').AsString;
  ACBrNFe.Configuracoes.Geral.CSC    := dm.sqlEmpresa.FieldByName('TOKEN').AsString;
  ...
  if (ECFAtual = 'NFCE DARUMA')   then   ACBrPosPrinter.Modelo := ppEscDaruma;
  if (ECFAtual = 'NFCE ELGIN')    then   ACBrPosPrinter.Modelo := ppEscElgin;

  ACBrPosPrinter.Device.Porta    := dm.SQLTerminalAtivoTERMA5ECFPORTACOM.Value;
  ACBrPosPrinter.Device.Baud     := dm.SQLTerminalAtivoECF_VELOC.Value;
...
begin
 ...
 { if dm.SQLTerminalAtivoTERMA60MENSAGEMPDV.Value <> '' then
  ...
  if FileExists('Tef_Dial.txt') then
    begin
      { Adilson, comentei essa rotina pq no Superbom aconteceu do cliente ir embora e o sistema travou e ao reiniciar cancelou a transação na auttar!
        ...
        begin
          ...
          if (not ImportandoPreVenda) and (ProdutoComplemento = 'S') then
            begin
              ...
              if FileExists('Tulipa.txt') then
                DescrLivreProd := SQLProdutoPRODA30ADESCRREDUZ.Value;
              FormTelaDescricaoTecnicaProduto.Memo1.Lines.Text := DescrLivreProd;
              FormTelaDescricaoTecnicaProduto.Memo1.SelStart := length(FormTelaDescricaoTecnicaProduto.Memo1.Lines.Text)+2;
              FormTelaDescricaoTecnicaProduto.ShowModal;
              if FormTelaDescricaoTecnicaProduto.ModalResult = MrOk then
                begin
                  DescrLivreProd := FormTelaDescricaoTecnicaProduto.Memo1.Lines.Text ;
  ...
  case Key of
     ...
     VK_F3:begin//FECHAMENTO DE VENDA
             ...
             if (ECFAtual = 'SWEDA_TERMICA') and (not FileExists('Confirma.txt')) then
              ...
              if (ECFAtual = 'DARUMA FS345') and (not FileExists('Confirma.txt')) then
                begin
                  try
                    dm.SubTotal_ECF := '00000000000000';
          ...
          'X':begin
                if (ECFAtual <> '') then
                  begin
                    ...
                    if (copy(ECFAtual,1,4) = 'NFCE') then
                      begin
                         ACBrNFe.DANFE.vTroco    := VarValorTroco;
                       { ACBrNFe.DANFE.vTribFed  := VarValorTroco;
                         ACBrNFe.DANFE.vTribEst  := VarValorTroco; }

                        // Cria o arquivo XML
                        LblInstrucoes.Caption := 'Aguarde... Montando Cupom Eletronico...';
                        LblInstrucoes.Update;
                        sXML := Gerar_NFCe(IDReimprimir);
                        // Envia o arquivo XML
                        LblInstrucoes.Caption := 'Aguarde... Transmitindo Cupom Eletronico...';
                        LblInstrucoes.Update;
                        ...
                        {Gravar nro da nota na venda}
                        SQLImpressaoCupom.Edit;
                        SQLImpressaoCupom.fieldbyname('CUPOINRO').Value := NumNFe;
                        SQLImpressaoCupom.Post;

                        try
                          if not ACBrNFe.Enviar('1', True, True) then
                            begin
                              if Pergunta('SIM','Tentar Transmitir Novamente?') then
                                begin
                                  if not ACBrNFe.Enviar('1', True, True) then
                                    begin
                                      if Pergunta('SIM','Tentar Transmitir Novamente?') then
                          ...
                          {Gravar Retorno da nota na venda}
                          SQLImpressaoCupom.Edit;
                          ...
                          SQLImpressaoCupom.fieldbyname('PROTOCOLO').Value   := ACBrNFe.WebServices.Retorno.Protocolo;
                          SQLImpressaoCupom.fieldbyname('RECIBO').Value      := ACBrNFe.WebServices.Retorno.Recibo;
                          SQLImpressaoCupom.Post;
                          SQLImpressaoCupom.Close;
                        finally
                          application.ProcessMessages;
                        ...
                        // Cupom Eletronico
                        LblInstrucoes.Caption := 'Aguarde... Finalizando Cupom Eletronico...';
                        LblInstrucoes.Update;
                      ...
                      begin
                        ...
                        if not SQLProvedorCartao.IsEmpty then
                          begin
                            ...
                            {VERIFICA SE O CRÉDITO DO CARTÃO FOI APROVADO}
                            if not VerificaCartaoCredito then
                              begin
                                InformaG('Transação TEF não completada!') ;
                                if Pergunta('SIM','Tentar Novamente?') then
                                  begin
                                    if not VerificaCartaoCredito then
                                      begin
                                        InformaG('Transação TEF não completada!') ;
      ...
      begin
        if (ssAlt in Shift) and (EntradaDados.text = '') then
          case Upcase(Char(Key)) of
            ...
            'X':begin
                  {Reimprimir NFCe Cupom Eletronico}
                  IDReimprimir := '';
                  IDReimprimir := InputBox('','Informar ID do Cupom Eletronico?',IDReimprimir);
                  if IDReimprimir <> '' then
                    begin
                      SQLImpressaoCupom.Close;
                      SQLImpressaoCupom.RequestLive := False;
                      SQLImpressaoCupom.SQL.Clear;
                      SQLImpressaoCupom.SQL.Add('Select CUPODEMIS, CHAVEACESSO From CUPOM Where CUPOA13ID ="'+IDReimprimir+'"');
                      SQLImpressaoCupom.Open;
                      sXML := SQLImpressaoCupom.fieldbyname('CHAVEACESSO').Value;
                      if sXML <> '' then
                        begin
                          Inicia_NFe;
                          ACBrNFe.NotasFiscais.Clear;
                          ACBrNFe.NotasFiscais.LoadFromFile('..\NFCe\'+
                                                            FormatDateTime('yyyymm',SQLImpressaoCupom.FieldByName('CUPODEMIS').Value) +
                                                            '\'+sXML+'-NFe.xml');
                          ACBrNFe.NotasFiscais[0].Imprimir;
                        end;
                    end;
    ...
    begin
      if SQLItensVendaTemp.Locate('NUMITEM',IntToStr(StrToInt(EntradaDados.Text)),[]) then
        begin
          ...
          Application.CreateForm(TFormTelaDescricaoTecnicaProduto,FormTelaDescricaoTecnicaProduto);
          FormTelaDescricaoTecnicaProduto.Memo1.Lines.Text := SQLItensVendaTempDESCRICAOTEC.Value;
          FormTelaDescricaoTecnicaProduto.ShowModal;
          if FormTelaDescricaoTecnicaProduto.ModalResult = MrOk then
            begin
              SQLItensVendaTemp.Edit;
              SQLItensVendaTempDESCRICAOTEC.Value := FormTelaDescricaoTecnicaProduto.Memo1.Lines.Text ;
              SQLItensVendaTemp.Post;
              SQLItensVendaTemp.Last;
              FormTelaDescricaoTecnicaProduto.Close;
            end;
...
procedure TFormTelaItens.GridItensColEnter(Sender: TObject);
begin
  EntradaDados.SetFocus;
end;
...
begin
  ...
  DM.SQLPedidoVendaPDVDN2VLRDESCPROM.Value := 0 ;
  DM.SQLPedidoVendaPDVDN2VLRDESC.Value     := TotalDescItens ;

  DM.SQLPedidoVendaPDVDN2TOTPROD.Value     := CurSubTotal.Value - TotalDescItens;
  DM.SQLPedidoVendaPDVDN2TOTPED.Value      := CurSubTotal.Value - TotalDescItens;
...
procedure TFormTelaItens.ACBrNFeStatusChange(Sender: TObject);
begin
