...
uses
  ...
  EDBNum, CurrEdit, Placemnt, RxLookup, Serial, AdvOfficeStatusBar,
...
type
  ...
  TFormCadastroNotaFiscalItem = class(TFormCadastroTEMPLATE)
    ...
    SQLUnidade2UNIDN3FATORCONV: TBCDField;
    ...
    procedure ImportarProdutosdoColetorBematechDC20001Click(
      Sender: TObject);
...
Begin
  ...
  Begin
    ...
    begin
        ...
        Begin
          SQLTemplateNFITN2BASESUBS.asFloat   := StrToFloat(FormatFloat('0.00',(((SQLTemplateNFITN2VLRUNIT.asFloat*SQLTemplateNFITN3QUANT.asFloat)-SQLTemplateNFITN2VLRDESC.AsFloat) + SQLTemplateNFITN2VLRFRETE.AsFloat + SQLTemplateNFITN2VLRIPI.AsFloat  + SQLTemplateNFITN2OUTRASDESP.AsFloat) +
          ((((SQLTemplateNFITN2VLRUNIT.asFloat*SQLTemplateNFITN3QUANT.asFloat)-SQLTemplateNFITN2VLRDESC.AsFloat) + SQLTemplateNFITN2VLRFRETE.AsFloat + SQLTemplateNFITN2VLRIPI.AsFloat + SQLTemplateNFITN2OUTRASDESP.AsFloat) * (xMVA/100))) );
          // SQLTemplateNFITN2VLRSUBS.asFloat    := StrToFloat(FormatFloat('0.00',(SQLTemplateNFITN2BASESUBS.value * (StrToFloat(Aliquotainterna)/100)) -
            SQLTemplateNFITN2VLRSUBS.asFloat    := StrToFloat(FormatFloat('0.00',((SQLTemplateNFITN2BASESUBS.value * SQLTemplateNFITN2PERCSUBS.value /100)) -
          ((((SQLTemplateNFITN2VLRUNIT.asFloat*SQLTemplateNFITN3QUANT.asFloat)-SQLTemplateNFITN2VLRDESC.AsFloat) + SQLTemplateNFITN2VLRFRETE.AsFloat ) * (nResultICMS.nICMS / 100)) ));
        End Else
        Begin
...
begin
  ...
  if (SQLTemplateNOFIA2CSTPIS.AsString = '01') or (SQLTemplateNOFIA2CSTPIS.AsString = '02') or (SQLTemplateNOFIA2CSTPIS.AsString = '03') or
     (SQLTemplateNOFIA2CSTPIS.AsString = '04') or (SQLTemplateNOFIA2CSTPIS.AsString = '05') or (SQLTemplateNOFIA2CSTPIS.AsString = '06') then
    begin
...
begin
  ...
  begin
    nVF:= (((((((100 + SQLTemplateNFITN2PERCIPI.Value) + (((100 + SQLTemplateNFITN2PERCIPI.Value)) * (SQLTemplateNFITN2PERCSUBS.asFloat / 100))) *
    (SQLTemplateNFITN2PERCICMS.value / 100)) ) -  SQLTemplateNFITN2PERCICMS.value ) / 100 ) + 1 ) + (SQLTemplateNFITN2PERCIPI.Value / 100);
...
procedure TFormCadastroNotaFiscalItem.SQLTemplateUNIDICODChange(
  Sender: TField);
begin
  inherited;
  if SQLTemplatePRODICOD.AsString <> '' then
  ...
  // Recalcula o Total
  if (SQLTemplateNFITN2VLRUNIT.Value > 0) and (SQLTemplateNFITN3QUANT.Value > 0)
    and (SQLTemplate.State in dsEditModes) then
...
procedure TFormCadastroNotaFiscalItem.MnImportarProdutospelaNotadeEntradaClick(
  Sender: TObject);
begin
  inherited;
  SQLTemplate.DisableControls;
...
procedure TFormCadastroNotaFiscalItem.ImportarProdutosdoColetorBematechDC20001Click(
  Sender: TObject);
var
  ...
  HouveErro : Boolean;
begin
  if DSMasterTemplate.DataSet.FieldByName('NOFICSTATUS').AsString <> 'A' then exit; // Aborta processo
...
procedure TFormCadastroNotaFiscalItem.ImportarProdutosdoColetorPalmOne1Click(
  Sender: TObject);
var
  Origem, Destino, CodProduto, CodInterno, vQtde, sCampo : String;
  LinhaAtual, NroLinhas, I, ContadorCampos, Resultado : integer;
