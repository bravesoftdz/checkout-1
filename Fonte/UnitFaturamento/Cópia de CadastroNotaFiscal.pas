...
begin
  If SQLTemplate.FindField('NOFICSTATUS').asString<>'A' Then
    Begin
      Informa('Você não pode excluir uma nota fiscal "Encerrada" ou "Cancelada"!');
      Abort;
    End;
  inherited;
end;
  ...
  If (StatusNovo = 'E') and (DM.SQLConfigVenda.FindField('CFVECMOVESTOQUENF').asString='S') and (EntradaSaida <> 'N') Then
    Begin
      ...
      if SQLContasReceberCTRCDEMIS.Value = SQLContasReceberCTRCDVENC.Value then
        if Pergunta('NAO','A Nota atual possui um valor À VISTA. Deseja quitar automaticamente?') Then
          Begin
...
{  if Not VerificaDadosProduto then
 ...
 // Linha :=   '<SDTNFE001 xmlns="urn:migrategnfe-schema">'                                            + CHR(13) +
  ...
  Linha :=   '    <ide>'                                                                                        + CHR(13) +
           ...
           {  if not SQLTemplateNOFIDSAIDAENTRADA.IsNull then
             ...
             '        <finNFe>1</finNFe>'                                                                       + CHR(13) + // Finalidade de emissão, 1- NF-e normal/ 2-NF-e complementar / 3 – NF-e de ajuste notas
 ...
 { Linha :=   '        <NFRefcolns:NFRef xmlns:NFRefcolns="NF-e" xmlns="">'                                                      + CHR(13) +
             ...
             '                <mod_refNFE>55</mod_refNFE>'                                                                      + CHR(13) + // Modelo do Documento Fiscal, Informar o código do modelo do Documento fiscal: 01 – modelo 01
...
begin
  ...
  begin
    ShowMessage('Nome do Destinatário Inválido. Verifique!'+#13#10+'Nota Fiscal Nº: '+SQLTemplateNOFIINUMERO.AsString);
    Result := False;
    Exit;
  end;
