...
uses
  ...
  cxDBEdit, dxSkinBlack,dxSkinBlue, dxSkinCaramel, dxSkinCoffee, dxSkinDarkRoom, dxSkinDarkSide,
...
type
  TFormCadastroNotaFiscal = class(TFormCadastroTEMPLATE)
    ...
    SQLNotaFiscalItemNOFIN3PERCCOFINS: TBCDField;
    SQLNotaFiscalItemNOFIN3VLRCOFINS: TBCDField;
    SQLNotaFiscalItemNOFIA2CSTPIS: TStringField;
    SQLNotaFiscalItemNOFIN3PERCPIS: TBCDField;
    SQLNotaFiscalItemNOFIN3VLRPIS: TBCDField;
...
//    procedure ImprimirDuplicatas1Click(Sender: TObject);
    ...
    procedure ReabrirNFeEncerradaNaoMovimentaEstoque1Click(
      Sender: TObject);
...
var
  ...
  vBC_Total, vICMS_Total, vBCST_Total, vICMSST_Total, vFrete_Total, vSeg_Total, vDesc_total, vII_Total,
  vIPI_Total, vPIS_Total, vCOFINS_Total, vOutro_Total, vNF_Total, vST_Total : Extended;
...
uses CadastroNotaFiscalFinanceiro, CadastroNotaFiscalItem, UnitLibrary,
  ...
  CadastroGraficaOrdem, CadastroNotaFiscalReferenciada, pcnConversaoNFe,
  ...
  begin
    ...
    while not SQLPedidoVendaItem.eof do
      begin
        ...
        TOTAL := Total + (SQLPedidoVendaItem.FieldByName('PVITN2VLRUNIT').AsFloat * (SQLPedidoVendaItem.FieldByName('PVITN3QUANT').AsFloat*
                                                                                     SQLUnidadeUNIDN3FATORCONV.Value));
      ...
      while not SQLNotaFiscalItens.Eof do
        begin
          if not TestaNotaCompraMovimentoEstoque('',  '', DataSet.FindField('NOFIA13ID').AsString,
                                                 SQLNotaFiscalItens.FindField('PRODICOD').AsString,SQLNotaFiscalItens.FindField('NFITN3QUANT').AsFloat) then
...
begin
  if (DM.PrevendaTerminalStr <> '') and (DM.PrevendaCodigoStr <> '') then
    begin
      ...
      SQLTemplateOPESICOD.AsVariant           := Dm.SQLConfigVendaOPESICODCUPOM.AsVariant;
      SQLTemplateSERIA5COD.AsVariant          := SQLLocate('OPERACAOESTOQUE','OPESICOD','SERIA5COD',Dm.SQLConfigVendaOPESICODCUPOM.AsString);
      SQLTemplateNOFIDEMIS.AsString           := FormatDateTime('dd/mm/yyyy',Now);
...
begin
  ...
  if SQLOperacaoEstoque.Locate('OPESICOD',DM.SQLConfigVendaOPESICODVENDCONSIG.AsString,[]) then
    begin
...
begin
  ...
  if FileExists(sXML) then
    begin
      if (Trim(sqlempresaEMPRA50EMAILHOST.Value) = EmptyStr) or
            (Trim(sqlempresaEMPRA75EMAILUSUARIO.Value) = EmptyStr) or
               (Trim(sqlempresaEMPRA50EMAILSENHA.Value) = EmptyStr) then
      begin
...
var PathPastaMensal, idLote, vAux, sXML, sXMLCancelamento : String;
    Point : TBookmark;
    InfoRetorno : TInfoRetornoUser;
begin
  inherited;
  ...
  if FileExists(sxml) then
    begin
      ...
      if (ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.cStat = 135) or
         (ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.cStat = 136) then
        begin
          SQLTemplate.BeforeEdit := nil;
          SQLTemplate.BeforePost := nil;
          SQLTemplate.Edit;
...
begin
  ...
  vII_Total     := 0;  vPIS_Total  := 0; vCOFINS_Total := 0;
  TemDifal := False;
  ...
  begin
    ...
    {Finalidade Emissao}
    if SQLTemplateNOFICFINALIDADE.Value = '1' Then Ide.finNFe  := fnNormal;
    if SQLTemplateNOFICFINALIDADE.Value = '2' Then Ide.finNFe  := fnComplementar;
    if SQLTemplateNOFICFINALIDADE.Value = '3' Then Ide.finNFe  := fnAjuste;
    if SQLTemplateNOFICFINALIDADE.Value = '4' Then Ide.finNFe  := fnDevolucao;
              ...
              begin
                ...
                if TemDifal then
                  begin
                    ...
                    if FormatDateTime('yyyy',sqltemplateNOFIDEMIS.Value) = '2016' then ICMSUFDest.pICMSInterPart  :=  40;
                    if FormatDateTime('yyyy',sqltemplateNOFIDEMIS.Value) = '2017' then ICMSUFDest.pICMSInterPart  :=  60;
                    if FormatDateTime('yyyy',sqltemplateNOFIDEMIS.Value) = '2018' then ICMSUFDest.pICMSInterPart  :=  80;
                    if FormatDateTime('yyyy',sqltemplateNOFIDEMIS.Value) = '2019' then ICMSUFDest.pICMSInterPart  := 100;
    ...
    Total.ICMSTot.vNF   :=  Total.ICMSTot.vProd  -
                            Total.ICMSTot.vDesc  +
                            Total.ICMSTot.vST    +
...
procedure TFormCadastroNotaFiscal.ReabrirNFeEncerradaNaoMovimentaEstoque1Click(
  Sender: TObject);
begin
  inherited;
...
procedure TFormCadastroNotaFiscal.FecharNotaAbertaNoMovimentaEstoque1Click(
  Sender: TObject);
begin
  inherited;
  Dm.SQLTemplate.SQL.Text := 'Update NOTAFISCAL set NOFICSTATUS = :xStatus where NOFIA13ID = :xid';
...
procedure TFormCadastroNotaFiscal.EncerrarNotasemSequencia1Click(
  Sender: TObject);
begin
  inherited;
  SQLTemplate.first;
...
procedure TFormCadastroNotaFiscal.MnImportarDadosdoPedidodeVendaClick(
  Sender: TObject);
begin
  inherited;
