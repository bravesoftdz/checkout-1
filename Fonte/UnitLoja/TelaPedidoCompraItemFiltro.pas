...
type
  TFormTelaPedidoCompraItemFiltro = class(TFormTelaGeralModalTemplate)
    ...
    zProdSaldoEMPRICOD: TIntegerField;
    zProdSaldoPSLDN3QTDE: TFloatField;
    zComprasNOCPA30NRO: TStringField;
    Label3: TLabel;
    Label22: TLabel;
    Label23: TLabel;
    zProdSaldoPSLDN3QTDMIN: TFloatField;
    Panel1: TPanel;
...
begin
 ...
 { if not CkPeriodoVendas.Checked then
    begin
      // Cria Campo com a Qtd Total de Venda
      sql := sql +  ', (Select SUM(CUPOMITEM.CPITN3QTD) from CUPOMITEM where (CUPOMITEM.CPITCSTATUS=''A'') and (CUPOMITEM.CPITN3QTD>0) and (CUPOMITEM.EMPRICOD='+EmpresaPadraoPedidosCompra+')';
      sql := sql +  ' and (CUPOMITEM.PRODICOD=PRODUTO.PRODICOD) and (CUPOMITEM.REGISTRO>= '' '+FormatDateTime('mm/dd/yyyy',DE.Date)+' '' and CUPOMITEM.REGISTRO<='' '+FormatDateTime('mm/dd/yyyy',ATE.Date)+' '') ) as QTD ';
    end;}
