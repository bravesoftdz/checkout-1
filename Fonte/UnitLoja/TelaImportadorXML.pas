...
type
  ...
  TFormTelaImportadorXML = class(TFormTelaGeralTEMPLATE)
    ...
    SQLNFSEFAZVALORTOTAL: TBCDField;
...
begin
  ...
  if dm.SQLConfigCompraCFCOCPREVREALNFCOMPRAFIN.Value <> '' then
    result := dm.SQLConfigCompraCFCOCPREVREALNFCOMPRAFIN.Value
  else
    begin
      result :='';
...
begin
  ...
  if dm.SQLConfigCompraPORTICOD.AsString <> '' then
    result := dm.SQLConfigCompraPORTICOD.Value
  else
    begin
      result :=0;
...
begin
  ...
  if dm.SQLConfigCompraTPDCICOD.AsString <> '' then
    result := dm.SQLConfigCompraTPDCICOD.Value
  else
    begin
      result :=0;
