...
type
  TFormTelaPlanoContas = class(TFormTelaGeralTEMPLATE)
    ...
    procedure BtApagarContasClick(Sender: TObject);
  private
...
begin
  for I:=1 To 10 do
    begin
      Hab:=(I<=DM.SQLConfigContaCFGCNIVEIS.AsInteger);
...
begin
  ...
  While Codigo[Cont]<> '' Do
    Begin
      If (DM.SQLConfigContaCFGCA30MASCARA.asString[Cont1]='_') Then
        Begin
          Resposta:=Resposta+Codigo[Cont];
          Inc(Cont);
        End
      else
        if DM.SQLConfigContaCFGCA30MASCARA.asString[Cont1] <> '' then
          Resposta:=Resposta+'.'
        else
          Inc(Cont);
      Inc(Cont1);
...
Begin
  ContaGrau:=0;
  Grau:=1;
  Tamanho:=DM.SQLConfigContaCFGCINIVEL1.asInteger;
...
Begin
  ...
  DM.SQLPlanodeContas.MacroByName('MFiltro').asString:='PLCTA15COD='''+Copy(Codigo,1,MemPos-1)+Tamanho_Grau(IntToStr(Aux+1),ContaGrau)+'''';
  DM.SQLPlanodeContas.Close;
  DM.SQLPlanodeContas.Open;
  DM.SQLPlanodeContas.First;
  If (ContaGrau=DM.SQLConfigContaCFGCNIVEIS.AsInteger) AND (DM.SQLPlanodeContas.Eof) Then
    Incremento:=1
  Else
    Incremento:=1;
  Sair := False;
...
Var
  ...
  Sair : Boolean;
Begin
  If Testa_Grau(Codigo)+1=DM.SQLConfigContaCFGCNIVEIS.asInteger Then
    Incremento:=1
  Else
    Incremento:=1;
  Aux:=1;
...
begin
  ...
  if Not DM.SQLPlanodeContas.Eof Then
    Begin
      ...
      If (Arvore.Selected.Level+1=DM.SQLConfigContaCFGCNIVEIS.asInteger) OR (Arvore.Selected.HasChildren) Then
...
begin
  ...
  If Arvore.Items.Count<>0 Then
    begin
      ...
      If (Arvore.Selected.Level+1=DM.SQLConfigContaCFGCNIVEIS.AsInteger) OR (DM.SQLPlanodeContasPLCTCANALSINT.asString='A') Then
...
begin
  ...
  DM.SQLPlanodeContasPLCTA60DESCR.asString:='';
...
begin
  ...
  NomeEdit.DataSource      := DM.DSSQLPlanodeContas ;
  CodReduzido.DataSource   := DM.DSSQLPlanodeContas ;
  CodigoEdit.DataSource    := DM.DSSQLPlanodeContas ;
  RadioSaldo.DataSource    := DM.DSSQLPlanodeContas ;
  RadioConta.DataSource    := DM.DSSQLPlanodeContas ;

  If (Arvore.Items.Count = 0) then
    Begin
      DM.SQLConfigConta.Close;
      DM.SQLPlanodeContas.Close;
...
begin
  ...
  If (DM.SQLPlanodeContas.State in DsEditModes) Then
    If DM.SQLPlanodeContas.FindField('PLCTA15COD').AsVariant<>Null Then
      DM.SQLPlanodeContas.Post
    Else
      DM.SQLPlanodeContas.Cancel;
...
procedure TFormTelaPlanoContas.RadioSaldoExit(Sender: TObject);
begin
  inherited;
  If (DM.SQLPlanodeContas.State=dsEdit) OR (DM.SQLPlanodeContas.State=dsInsert) Then
    Begin
...
procedure TFormTelaPlanoContas.RadioContaExit(Sender: TObject);
begin
  inherited;
  If (DM.SQLPlanodeContas.State=dsEdit) OR (DM.SQLPlanodeContas.State=dsInsert) Then
    Begin
...
procedure TFormTelaPlanoContas.BtApagarContasClick(Sender: TObject);
begin
  inherited;
  if Application.MessageBox('Deseja apagar todas as contas cadastradas?','Atenção',MB_YesNo+MB_IconQuestion)=Id_Yes then
    begin
      dm.SQLPlanodeContas.close;
      dm.SQLPlanodeContas.MacroByName('MFiltro').Value := '0=0';
      dm.SQLPlanodeContas.open;
      dm.SQLPlanodeContas.First;
      while not dm.SQLPlanodeContas.eof do
        dm.SQLPlanodeContas.Delete;
      Close;
    end;
end;
