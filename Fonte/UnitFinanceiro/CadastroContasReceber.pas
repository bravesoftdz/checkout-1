...
uses
  ...
  ppCtrls, ppPrnabl, ppClass, ppCache, ppBands, ppComm, ppRelatv, ppProd,
  ppReport, AdvOfficeStatusBar, AdvOfficeStatusBarStylers, AdvPanel,
  ppStrtch, ppMemo, Placemnt;
...
uses  CadastroCliente, DataModulo, CadastroTipoDocumento,
  ...
  TelaAssistenteLancamentoContasReceber, TelaBaixaContasReceberAVista,
  TelaAssistenteContratosMensais, TelaSplash;
...
begin
  ...
  SQLTemplateCTRCA2MESCOMP.Value                   := FormatDateTime('MM',DM.DataBaseSistema);
    ...
    begin
      if not FormTelaAssistenteLancamentoContasReceber.RxTable.IsEmpty then
        begin
          ...
          dm.sqlconsulta.sql.Text := 'select max(CTRCICOD) from contasreceber where empricod='+dm.SQLTerminalAtivoEMPRICOD.AsString+' and termicod='+dm.SQLTerminalAtivoTERMICOD.AsString;
          dm.sqlconsulta.open;
          ...
          while not FormTelaAssistenteLancamentoContasReceber.RxTable.Eof do
            begin
              SQLTemplate.Append;
              SQLTemplateEMPRICOD.Value                    := dm.SQLTerminalAtivoEMPRICOD.Value;
              SQLTemplateTERMICOD.Value                    := dm.SQLTerminalAtivoTERMICOD.Value;
              SQLTemplateCTRCICOD.Value                    := codigo;
              SQLTemplateCTRCA13ID.Value                   := FormatFloat('###000',dm.SQLTerminalAtivoEMPRICOD.Value)+FormatFloat('###000',dm.SQLTerminalAtivoTERMICOD.Value)+FormatFloat('######000000',codigo);
...
procedure TFormCadastroContasReceber.SQLTemplateBeforePost(
  DataSet: TDataSet);
begin
...
procedure TFormCadastroContasReceber.SQLTemplateAfterPost(
  DataSet: TDataSet);
begin
  inherited;
...
procedure TFormCadastroContasReceber.mnNotaPromissoriaClick(
  Sender: TObject);
begin
  inherited;
  ppPromissoria.Print;
...
procedure TFormCadastroContasReceber.ppHeaderBand1BeforePrint(
  Sender: TObject);
begin
  inherited;
  ...
  ppCidadeEmpresa.Caption   := dm.SQLEmpresaEMPRA60CID.AsString +
                               ', '   + FormatDateTime('dd',SQLTemplateCTRCDEMIS.Value)+
                               ' de ' + FormatDateTime('mmmm',SQLTemplateCTRCDEMIS.Value)+
                               ' de ' + FormatDateTime('yyyy',SQLTemplateCTRCDEMIS.Value);
  ppValor.Caption      := FormatCurr('##0.00',SQLTemplateCTRCN2VLR.Value);
  ppVencimento.Caption := 'Vencimento em ' +
                          FormatDateTime('dd',SQLTemplateCTRCDVENC.Value)+
                          ' de ' + FormatDateTime('mmmm',SQLTemplateCTRCDVENC.Value)+
                          ' de ' + FormatDateTime('yyyy',SQLTemplateCTRCDVENC.Value);

  ppLogo.Picture.LoadFromFile('Bmp\Logo PDV.jpg');
end;

procedure TFormCadastroContasReceber.DBGridListaDrawColumnCell(
...
procedure TFormCadastroContasReceber.MnAssistenteLancContratosMensaisClick(
  Sender: TObject);
begin
  inherited;
    ...
    begin
      if not FormTelaAssistenteContratosMensais.RxTable.IsEmpty then
        begin
          ...
          FormSplash.Show;
          FormSplash.lbDados.Caption := 'Aguarde!! Gerando Contas a Receber...'; FormSplash.lbDados.Update;
          SQLTemplate.DisableControls;
          SQLTemplate.BeforePost   := nil;
          SQLTemplate.OnCalcFields := nil;
...
procedure TFormCadastroContasReceber.ppHeaderBand2BeforePrint(
  Sender: TObject);
var
  mes, ano : integer;
begin
  ...
  if ckImprimeMesReferencia.Checked then
    begin
      ...
      if (mes = 1) then ppMesReferencia.Caption := 'REF. DEZEMBRO/'+ IntToStr(ano-1);
      if (mes = 2) then ppMesReferencia.Caption := 'REF. JANEIRO/'+ IntToStr(ano);
      if (mes = 3) then ppMesReferencia.Caption := 'REF. FEVEREIRO/'+ IntToStr(ano);
      if (mes = 4) then ppMesReferencia.Caption := 'REF. MARCO/'+ IntToStr(ano);
      if (mes = 5) then ppMesReferencia.Caption := 'REF. ABRIL/'+ IntToStr(ano);
      if (mes = 6) then ppMesReferencia.Caption := 'REF. MAIO/'+ IntToStr(ano);
      if (mes = 7) then ppMesReferencia.Caption := 'REF. JUNHO/'+ IntToStr(ano);
      if (mes = 8) then ppMesReferencia.Caption := 'REF. JULHO/'+ IntToStr(ano);
      if (mes = 9) then ppMesReferencia.Caption := 'REF. AGOSTO/'+ IntToStr(ano);
      if (mes =10) then ppMesReferencia.Caption := 'REF. SETEMBRO/'+ IntToStr(ano);
      if (mes =11) then ppMesReferencia.Caption := 'REF. OUTUBRO/'+ IntToStr(ano);
      if (mes =12) then ppMesReferencia.Caption := 'REF. NOVEMBRO/'+ IntToStr(ano);
  ...
  ppNomeCNPJEmpresaRecibo.Caption := dm.SQLEmpresaEMPRA60RAZAOSOC.AsString +
                               ' CNPJ: '+ dm.SQLEmpresaEMPRA14CGC.AsString;
  ppNomeCNPJEmpresaRecibo1.Caption :=   ppNomeCNPJEmpresaRecibo.Caption;

  ppCidadeEmpresaRecibo.Caption   := dm.SQLEmpresaEMPRA60CID.AsString +
                               ', '   + FormatDateTime('dd',now)+
                               ' de ' + FormatDateTime('mmmm',now)+
                               ' de ' + FormatDateTime('yyyy',now);
  ...
  ppVencimentoRecibo.Caption := 'Vencimento em ' +
                          FormatDateTime('dd',SQLTemplateCTRCDVENC.Value)+
                          ' de ' + FormatDateTime('mmmm',SQLTemplateCTRCDVENC.Value)+
                          ' de ' + FormatDateTime('yyyy',SQLTemplateCTRCDVENC.Value);
...
procedure TFormCadastroContasReceber.CorrigeTotalRecebidoSafe1Click(
  Sender: TObject);
begin
  inherited;
  sqltemplate.First;
